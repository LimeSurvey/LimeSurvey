...
// Saleh AlMatrafe for arabic support.
// Mohamad Ali Golkar for persian and arabic shaping support.
...
if(!class_exists('TCPDF')) {
	 ...
	 */ 
	define('PDF_PRODUCER','TCPDF 3.0.014_PHP4 (http://www.tcpdf.org)');
	...
	*/
    ...
    /**
		...
		*/
		function SetFillColor($col1=0, $col2=-1, $col3=-1, $storeprev=false, $col4=-1) {
			// set default values
			if (!is_numeric($col1)) {
				$col1=0;
			}
		...
		*/
		function SetTextColor($col1=0, $col2=-1, $col3=-1, $storeprev=false, $col4=-1) {
			// set default values
			if (!is_numeric($col1)) {
				$col1=0;
			}
			...
			if ($arabic) {
				...
				for ($i=0; $i < $numchars; $i++) {
					...
					if ($unicode[$thischar['char']] == 'AL') {
						...
						// if laa letter
						if (($i > 0) AND ($prevchar['char'] == 1604) AND (in_array($thischar['char'], $alfletter))) {
							$arabicarr = $laa_array;
							$laaletter = true;
							$prevchar  = $charAL[($x-2)];
						} else {
						...
						if (($i > 0) AND (($i+1) < $numchars) AND
							...
							if (isset($unicode_arlet[$chardata[$i]['char']][2])) {
								// initial
								$chardata2[$i]['char'] = $unicode_arlet[$thischar['char']][2];
							}
						} elseif ((($i > 0) AND
							(($unicode[$prevchar['char']] == 'AL') OR ($unicode[$prevchar['char']] == 'NSM')) AND
