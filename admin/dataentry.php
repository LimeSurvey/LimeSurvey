...
if ($_SESSION['USER_RIGHT_SUPERADMIN'] == 1 || $actsurrows['browse_response'])
{
	...
	if ($subaction == "insert")
	{
		...
		if (tokenTableExists($thissurvey) && (!isset($_POST['token']) || !$_POST['token']))
		...
		elseif (tokenTableExists($thissurvey) && $lastanswfortoken == 'UnknownToken')
		{
		...
		elseif (tokenTableExists($thissurvey) && $lastanswfortoken != '')
		{
		...
		else
		{
			...
			//NOW SHOW SCREEN
			if (tokenTableExists($thissurvey) && 
...
//					while ($fnames[$i][3] == "M" && $question != "" && $question == $fnames[$i][2])
	...
	else
	{
		...
		if (tokenTableExists($thissurvey)) //Give entry field for token id 
		{
			$dataentryoutput .= "\t<tr>\n"
