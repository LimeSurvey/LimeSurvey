...
{    
    ...
    public function loadSurveyById($id) 
    {
        ...
        //Load groups
        $sql = 'SELECT g.* FROM '.db_table_name('groups').' AS g '.
               'WHERE g.sid = '.$intId.' '.
               'ORDER BY g.group_order;';
        $recordSet = db_execute_assoc($sql);
        $survey->groups = $recordSet->getArray(-1);
        $recordSet->close();
        
        //Load questions
        $sql = 'SELECT q.* FROM '.db_table_name('questions').' AS q '.
               'JOIN '.db_table_name('groups').' AS g ON q.gid = g.gid '.
               'WHERE q.sid = '.$intId.' AND q.language = \''.$lang.'\' '.
               'ORDER BY g.group_order, q.question_order;';
        $recordSet = db_execute_assoc($sql);
        $survey->questions = $recordSet->GetArray(-1);
        $recordSet->close();
        
        //Load answers
        $sql = 'SELECT DISTINCT a.* FROM '.db_table_name('answers').' AS a '.
               'JOIN '.db_table_name('questions').' AS q ON a.qid = q.qid '.
		       'WHERE q.sid = '.$intId.' AND a.language = \''.$lang.'\' '.
               'ORDER BY a.qid, a.sortorder;';
        $recordSet = db_execute_assoc($sql);
        $survey->answers = $recordSet->GetArray(-1);
        $recordSet->close();  
        
        //Load tokens                              
        $sql = 'SELECT t.* FROM '.db_table_name('tokens_'.$intId).' AS t;';
        $recordSet = db_execute_assoc($sql);
        $survey->tokens = $recordSet->GetArray(-1);
        $recordSet->close();
        
        //Load language settings
        $sql = 'SELECT * FROM '.db_table_name('surveys_languagesettings').' WHERE surveyls_survey_id = '.$intId.';';
        $recordSet = db_execute_assoc($sql);
        $survey->languageSettings = $recordSet->GetArray(-1);
        $recordSet->close(); 
    ...
    public function loadSurveyResults(Survey $survey, $minRecord = null, $maxRecord = null) 
    {
        ...
        $sql = 'SELECT * FROM '.db_table_name('survey_'.$survey->id);
        ...
        $survey->responses = $recordSet->GetArray(-1);
    }
}    
