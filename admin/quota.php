...
if($sumrows5['edit_survey_property'])
{
	...
	if ($action == "modqquota")
	{
		...
		$answerlist = getQuotaAnswers($qid,$surveyid);

		if (count($answerlist) > 0) // Available Question Types Only
		{
			foreach ($answerlist as $quotaanswer) // Cycle through avaiable answers to check quota settings submitted
			{
				if (isset($_POST['qquota_enabled_'.$quotaanswer['code']]) && $_POST['qquota_enabled_'.$quotaanswer['code']] == "yes")
				{ // Answer was ticked to enable quota

					// Sanatize Values

					$_POST['qquota_value_'.$quotaanswer['code']] = sanitize_int($_POST['qquota_value_'.$quotaanswer['code']]);
					$_POST['qquota_action_'.$quotaanswer['code']] = sanitize_int($_POST['qquota_action_'.$quotaanswer['code']]);


					if ($quotaanswer['rowexists'] == '1') // Quota entry exists, update entry.
					{
						$query = "UPDATE ".db_table_name('quota')." set enabled='1', action='{$_POST['qquota_action_'.$quotaanswer['code']]}', value='{$_POST['qquota_value_'.$quotaanswer['code']]}', action='{$_POST['qquota_action_'.$quotaanswer['code']]}' WHERE qid='$qid' and code='{$quotaanswer['code']}'";
						$result=$connect->Execute($query);
					} else
					{ // insert a new quota entry
						$query = "INSERT INTO ".db_table_name('quota')." (sid,qid,aid,code,value,action,enabled) VALUES ('$surveyid','$qid','{$quotaanswer['aid']}','{$quotaanswer['code']}','{$_POST['qquota_value_'.$quotaanswer['code']]}','{$_POST['qquota_action_'.$quotaanswer['code']]}','1')";
						$result=$connect->Execute($query);
					}

				} else { // Answer is not ticked for quota

					// Check if there is already a quota entry for this answer
					$query = "SELECT id FROM ".db_table_name('quota')." WHERE qid='{$qid}' AND code='{$quotaanswer['code']}'";
					$result = db_execute_assoc($query) or die($connect->ErrorMsg());

					if ($quotaanswer['rowexists'] == '1') // Quota entry exists
					{
						if (isset($_POST['qquota_value_'.$quotaanswer['code']]) && !empty($_POST['qquota_value_'.$quotaanswer['code']]))
						{ // Check if value is still set, make sure enabled is set to 0.
							$query = "UPDATE ".db_table_name('quota')." set enabled='0', action='{$_POST['qquota_action_'.$quotaanswer['code']]}', value='{$_POST['qquota_value_'.$quotaanswer['code']]}', action='{$_POST['qquota_action_'.$quotaanswer['code']]}' WHERE qid='$qid' and code='{$quotaanswer['code']}'";
							$result=$connect->Execute($query);
						} elseif (empty($_POST['qquota_value_'.$quotaanswer['code']]))
						{ // Delete entry if there is no value
							$query = "DELETE FROM ".db_table_name('quota')." WHERE qid='$qid' and code='{$quotaanswer['code']}'";
							$result=$connect->Execute($query);
						}
					}
		...
		// Refer to question quota view.
		$action = "quota";
	}

	if ($action == "quotasdfdfadsf" || returnglobal('viewqquota'))
	{

		$quotasoutput .= "\t<table width='100%' >\n"
		."<tr  >\n"
		."\t<td colspan='4' class='settingcaption'>\n"
		.$clang->gT("Question Quota")
		."\t</td>\n"
		."</tr>\n";

		$answerlist = getQuotaAnswers($qid,$surveyid);

		//die(print_r($answerlist));

		if (count($answerlist) > 0) // Available Question Types Only
		{
			$quotasoutput .=  "\t<tr><td colspan='5'><form name='modifyquestionquota' method='post' action='$scriptname'>\n"
			. "\t<input type='hidden' name='sid' value='$surveyid' />\n"
			. "\t<input type='hidden' name='gid' value='$gid' />\n"
			. "\t<input type='hidden' name='qid' value='$qid' />\n"
			. "\t<input type='hidden' name='viewqquota' value='Y' />\n"
			. "\t<input type='hidden' name='action' value='modqquota' />\n";


			$quotasoutput .= "<br /><table width='500' align='center' style='border: solid; border-width: 0px; border-color: #555555' cellspacing='0'>\n"
			."<thead align='center'>"
			."<tr bgcolor='#F8F8FF'>\n"
			."\t<td width='40%' align='center'><strong><font size='1' face='verdana' >\n"
			.$clang->gT("Answer")
			."\t</font></strong></td>\n"
			."\t<td width='20%'><strong><font size='1' face='verdana'>\n"
			.$clang->gT("Quota Enabled")
			."\t</font></strong></td>\n"
			."\t<td width='20%'><strong><font size='1' face='verdana'>\n"
			...
			."\t</font></strong></td>\n"
			."</tr></thead>"
			."<tbody align='center'><tr><td colspan='4'>&nbsp;</td></tr>";

			foreach ($answerlist as $quotaanswer)
			{
				$quotasoutput .= "<tr><td width='40%' align='center'>\n"
				."\t{$quotaanswer['Display']}";

				$quotasoutput .= "\t</td>\n"
				."\t<td width='20%'>\n"
				."\t<input type='checkbox' name='qquota_enabled_{$quotaanswer['code']}' value='yes'";

				if (isset($quotaanswer['enabled']) && $quotaanswer['enabled'] == 1) $vasummary .=" checked='checked'";

				$quotasoutput .= " />\n"
				."\t</td>\n";

				$quotasoutput .= "\t<td width='20%'>\n"
				."\t<input type='text' name='qquota_value_{$quotaanswer['code']}' maxlength='10' size='5' value=\"{$quotaanswer['value']}\" onkeypress=\" if(event.keyCode==13) {if (event && event.preventDefault) event.preventDefault(); document.getElementById('saveallbtn_$anslang').click(); return false;}\" />\n"
				."\t</td>";

				$quotasoutput .= "\t<td width='20%'>\n"
				."\t<select name='qquota_action_{$quotaanswer['code']}'>";

				$quotasoutput .= "<option value ='1' ";
				if((isset($quotaanswer['action']) && $quotaanswer['action'] == 1) || !isset($quotaanswer['action']))  $vasummary .= "selected='selected'";
				$quotasoutput .= ">".$clang->gT("Terminate Survey")."</option>";

				$quotasoutput .= "<option value ='2' ";
				if(isset($quotaanswer['action']) && $quotaanswer['action'] == 2)  $vasummary .= "selected='selected'";
				$quotasoutput .= ">".$clang->gT("Terminate Survey With Warning")."</option>";

				$quotasoutput .= "</select></td>\n</tr>";

			}

			$quotasoutput .= "\t<tr><td colspan='4'>&nbsp;</td></tr><tr><td colspan='3'><center><input type='submit' id='saveallbtn_$anslang' name='method' value='".$clang->gT("Save")."'  />"
			."</center></td></tr>\n";

		} else {

			$quotasoutput .= "\t<tr><td><br /><center>Sorry this question type is not supported yet.</center><br /></td></tr></table>\n";
