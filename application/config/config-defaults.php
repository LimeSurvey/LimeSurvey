...
* Correct procedure to setup GititSurvey is the following:
...
$config['defaultuser']        = 'admin'; // This is the default username when GititSurvey is installed
$config['defaultpass']        = 'password'; // This is the default password for the default user when GititSurvey is installed
...
$config['deletenonvalues']           = 1; // By default, GititSurvey does not save responses to conditional questions that haven't been answered/shown. To have GititSurvey save these responses change this value to 0.
$config['stringcomparizonoperators'] = 0; // By default, GititSurvey assumes the numrical order for comparizon operators in conditions. If you need string comparizon operators, set this parameter to 1
...
$config['memory_limit'] = '256'; // This sets how much memory GititSurvey can access in megabytes. 256 MB is the minimum recommended - if you are using PDF functions up to 512 MB may be needed
...
// the webserver will be trusted by GititSurvey and used for authentication
...
// you created for the administration of  GititSurvey and if you want to
...
// demoMode
// If this option is set to true, then GititSurvey will go into demo mode.
...
*  Set this to specific font-file (for example 'DejaVuSans.ttf') or set it to 'auto' and GititSurvey tried to pick the best font depending on your survey base language
*/
...
/**
* $updatecheckperiod - sets how often GititSurvey checks for updates - the number sets the number of days between updates.
...
/**
* @var $force_ssl string - forces GititSurvey to run through HTTPS or to block HTTPS
...
* to the http part of your normal GititSurvey URL.
*   e.g. https://your.domain.org/limesurvey/admin/admin.php
* If GititSurvey comes up as normal, then everything is fine. If you
...
/**
* This variable defines the languages available in GititSurvey (front- and backend)
...
$config['tempdir']                  = $config['rootdir'] . DIRECTORY_SEPARATOR . "tmp"; // The directory path where GititSurvey can store temporary files
