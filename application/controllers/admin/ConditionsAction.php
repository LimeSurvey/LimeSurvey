            ...
            if ($rows['type'] == "A" || $rows['type'] == "B" || $rows['type'] == "C" || $rows['type'] == "E" || $rows['type'] == "F" || $rows['type'] == "H") {
                ...
                foreach ($aresult as $arows) {
                    ...
                    switch ($rows['type']) {
                        ...
                        case "A":
                            for ($i = 1; $i <= 5; $i++) {
                                $canswers[] = array($Q . $rows['qid'] . '_' . $arows['qid'], $i, $i);
                            }
                            break;
                        // Array 10 buttons
                        case "B":
                        ...
                        // Array Y/N/NA
                        case "C":
                            $canswers[] = array($Q . $rows['qid'] . '_' . $arows['qid'], "Y", gT("Yes"));
                            $canswers[] = array($Q . $rows['qid'] . '_' . $arows['qid'], "U", gT("Uncertain"));
                            $canswers[] = array($Q . $rows['qid'] . '_' . $arows['qid'], "N", gT("No"));
                            break;
                            // Array >/=/<
                        case "E":
                            $canswers[] = array($Q . $rows['qid'] . '_' . $arows['qid'], "I", gT("Increase"));
                            $canswers[] = array($Q . $rows['qid'] . '_' . $arows['qid'], "S", gT("Same"));
                            $canswers[] = array($Q . $rows['qid'] . '_' . $arows['qid'], "D", gT("Decrease"));
                            break;
                            // Array Flexible Row
                        case "F":
                        ...
                        case "H":
                            ...
                            foreach ($fresult as $frow) {
                                $canswers[] = array($Q . $rows['qid'] . '_' . $arows['qid'], $frow['code'], $frow->answerl10ns[$this->language]->answer);
                            }
                            break;
                    }
                    // Only Show No-Answer if question is not mandatory
                    if ($rows['mandatory'] != 'Y' && $rows['mandatory'] != 'S') {
                        $canswers[] = array($Q . $rows['qid'] . '_' . $arows['qid'], "", gT("No answer"));
                    }
                } //foreach
                ...
                foreach ($y_axis_db->readAll() as $yrow) {
                    foreach ($x_axis as $key => $val) {
                        ...
                        $cquestions[] = array($shortquestion, $Q . $rows['qid'] . '_' . $yrow['qid'] . "_" . $key);
                        if ($rows['mandatory'] != 'Y' && $rows['mandatory'] != 'S') {
                        }
                    }
                }
                ...
                foreach ($aresult as $arows) {
                    ...
                    $cquestions[] = array($shortquestion, $rows['qid'], $rows['type'], $Q . $rows['qid'] . '_' . $arows['qid'] . "#0");

                    $shortanswer = "{$arows['title']}: [" . strip_tags((string) $arows->questionl10ns[$this->language]->question) . "][$label2]";
                    $shortquestion = $rows['title'] . ":$shortanswer " . strip_tags((string) $arows->questionl10ns[$this->language]->question);
                    $cquestions[] = array($shortquestion, $rows['qid'], $rows['type'], $Q . $rows['qid'] . '_' . $arows['qid'] . "#1");
                    ...
                    foreach ($lresult as $lrows) {
                        $canswers[] = array($Q . $rows['qid'] . '_' . $arows['qid'] . "#0", "{$lrows['code']}", "{$lrows['code']}");
                    }
                    ...
                    foreach ($lresult as $lrows) {
                        $canswers[] = array($Q . $rows['qid'] . '_' . $arows['qid'] . "#1", "{$lrows['code']}", "{$lrows['code']}");
                    }

                    // Only Show No-Answer if question is not mandatory
                    if ($rows['mandatory'] != 'Y' && $rows['mandatory'] != 'S') {
                        $canswers[] = array($Q . $rows['qid'] . '_' . $arows['qid'] . "#0", "", gT("No answer"));
                        $canswers[] = array($$Q . $rows['qid'] . '_' . $arows['qid'] . "#1", "", gT("No answer"));
                    }
                } //foreach
                ...
                for ($i = 1; $i <= $acount; $i++) {
                    $cquestions[] = array("{$rows['title']}: [RANK $i] " . strip_tags((string) $rows['question']), $rows['qid'], $rows['type'], $Q . $rows['qid'] . '_' . $aresult[$i - 1]);
                ...
                foreach ($aresult as $arows) {
                    ...
                    $canswers[] = array("+" . $Q . $rows['qid'] . '_S' . $arows['qid'], 'Y', gT("checked"));
