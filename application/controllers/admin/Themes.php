    ...
    public function templaterename()
    {
        ...
        if (empty($sNewName)) {
            throw new CHttpException(
                400, // bad request
                gT("New name can not be empty, maybe you try to use invalid caracter.")
            );
        }
        if (empty($sOldName)) {
            throw new CHttpException(
                400, // bad request
                gT("template name can not be empty, maybe you try to use invalid caracter.")
            );
        }
        ...
        /* We get there : template is renamed, rename it in DB */
        $oTemplate->renameTo($sNewName);
        /* reste to default if oit was current default */
