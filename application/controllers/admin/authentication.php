    ...
    public function forgotpassword()
    {
        ...
        if (!Yii::app()->request->getPost('action')) {
            ...
            if (($user === null) || ($user->uid != 1 && !Permission::model()->hasGlobalPermission('auth_db', 'read', $user->uid))) {
                ...
                $aData['message'] = '<br>' . sprintf(gt('If the username and email address is valid and you are allowed to use the internal database authentication a new password has been sent to you. Email is send only once in %d minutes'), \LimeSurvey\Models\Services\PasswordManagement::MIN_TIME_NEXT_FORGOT_PW_EMAIL) . '<br>';
            } else {
