...
class conditionsaction extends Survey_Common_Action
{
    public function index($subaction, $iSurveyID = null, $gid = null, $qid = null)
    {
        ...
        if (isset($postquestionscount) && $postquestionscount > 0) { //Build the array used for the questionNav and copyTo select boxes
...
.$method[trim($rows['method'])]
                            ...
                            $subaction == 'renumberscenarios' || $subaction == 'deleteallconditions' ||
                            $subaction == 'updatescenario' ||
                            $subaction == 'deletescenario' || $subaction == 'delete') { // show single condition action buttons in edit mode
                ...
                LimeExpressionManager::UpgradeConditionsToRelevance(null, $qid);
            } else { // no condition ==> disable delete all conditions button, and display a simple comment
...
."\t\t<input type='submit' value='".gT('Copy conditions')."' onclick=\"prepareCopyconditions(); return true;\" $disableCopyCondition/>\n"
        ...
        $subaction == 'editthiscondition' || $subaction == 'delete') {
            ...
            foreach ($cquestions as $cqn) { // building the @SGQA@ placeholders options
                if ($cqn[2] != 'M' && $cqn[2] != 'P') { // Type M or P aren't real fieldnames and thus can't be used in @SGQA@ placehodlers
                    $aViewUrls['output'] .= "\t\t<option value='@$cqn[3]@' title=\"".htmlspecialchars($cqn[0]).'"';
...
."</div>\n"
            ...
            if ($subaction == 'editthiscondition') { // in edit mode we read previous values in order to dusplay them in the corresponding inputs
                if (isset($_POST['EDITConditionConst']) && $_POST['EDITConditionConst'] != '') {
                ...
                } elseif (isset($_POST['EDITcanswers']) && is_array($_POST['EDITcanswers'])) { // was a predefined answers post
                ...
                } elseif (isset($_POST['cquestions']) && $_POST['cquestions'] != '') {
