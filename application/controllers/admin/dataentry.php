...
class dataentry extends Survey_Common_Action
{
    ...
    public function editdata($subaction, $id, $surveyid, $language = '')
    {
        ...
        if (Permission::model()->hasSurveyPermission($surveyid, 'responses', 'update')) {
            ...
            if (!Permission::model()->hasSurveyPermission($surveyid, 'responses', 'update')) { // if you are not survey owner or super admin you cannot modify responses
        ...
        if (Permission::model()->hasSurveyPermission($surveyid, 'responses', 'create')) {
            if ($subaction == 'insert' && Permission::model()->hasSurveyPermission($surveyid, 'responses', 'create')) {
                ...
                if (isset($_POST['token'])) {
                    ...
                    } elseif ($thissurvey['anonymized'] == 'Y') { // token exist but survey is anonymous, check completed state
                        if ($tokencompleted != '' && $tokencompleted != 'N') { // token is completed
                            $lastanswfortoken = 'PrivacyProtected';
                        }
                    } else { // token is valid, survey not anonymous, try to get last recorded response id
                ...
                if (tableExists('{{tokens_'.$thissurvey['sid'].'}}') && (!$_POST['token'])) {
                    ...
                    foreach ($fieldmap as $irow) {
                        $fieldname = $irow['fieldname'];
                        if (isset($_POST[$fieldname])) {
                            if ($_POST[$fieldname] == '' && ($irow['type'] == 'D' || $irow['type'] == 'N' || $irow['type'] == 'K')) { // can't add '' in Date column
                                // Do nothing
                            } elseif ($irow['type'] == '|') {
