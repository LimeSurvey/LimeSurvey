        ...
        if(bHasSurveyPermission($surveyid,'responses','create'))
        {
            ...
            if ($subaction != "upload")
            {
                ...
                else
                {
            		...
            		[<a href='".site_url('admin/survey/view/'.$surveyid)."'>".$clang->gT("Return to survey administration")."</a>]
            		</td></tr>
            		</table>";		
                }
            ...
            else
            {
                ...
                if (!@move_uploaded_file($_FILES['the_file']['tmp_name'], $the_full_file_path))
                {
                    ...
                    $vvoutput .= "<input type='submit' value='".$clang->gT("Back to Response Import")."' onclick=\"window.open('".site_url('admin/dataentry/vvimport/'.$surveyid)."', '_top')\">\n";
                ...
                foreach($bigarray as $row)
                {
                    if (trim($row) != "")
                    {
                        ...
                        if (!$result)
                        {
                            ...
                            ."<tr><td>".sprintf($clang->gT("Import Failed on Record %d"), $recordcount)
                            ."</td>\n</tr></table>\n";
                        }
                        else
                        {
