...
class statistics extends Survey_Common_Controller {
	...
	function action($surveyid, $subaction = null)
	{
		...
		foreach ($survlangs as $survlang)
		{
		    ...
		    if ($this->session->userdata('adminlang') == $survlang)
		    {
		...
		foreach ($filters as $flt)
		{
		    ...
		    switch ($flt[2])
		    {
		        ...
		        case ";":  //ARRAY (Multi Flex) (Text)
		            ...
		            {
		            	$row = array_values($row);
		                $fquery = "SELECT title, question FROM ".$this->db->dbprefix("questions")." WHERE parent_qid='$flt[0]' AND language='{$language}' AND scale_id=1 ORDER BY question_order";
