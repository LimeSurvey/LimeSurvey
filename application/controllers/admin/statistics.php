 ...
 *
 * $Id: statistics.php 11221 2011-10-19 20:47:40Z tmswhite $
 *
 */
...
class statistics extends Survey_Common_Action {
	...
	public function run($surveyid, $subaction = null)
	{
		...
		$statisticsoutput = "<form method='post' name='formbuilder' action='".$this->controller->createUrl("admin/statistics/surveyid/$surveyid")."#start'>\n";

		//Select public language file
		$query = "SELECT datestamp FROM {{surveys}} WHERE sid=$surveyid";
		 ...
		 * no -> $datestamp="N"
		 */
		$row=$result->read(); $datestamp=$row['datestamp'];
		...
		."<input type='button' value='".$clang->gT("Clear")."' onclick=\"window.open('".$this->controller->createUrl("admin/statistics/surveyid/$surveyid")."', '_top')\" />\n"
		."</p>";
		...
		foreach ($filters as $flt)
		{
		    ...
		    switch ($flt[2])
		    {
		        ...
		        case ";":  //ARRAY (Multi Flex) (Text)
		            ...
		            {
		            	$row = array_values($row);
		                $fquery = "SELECT title, question FROM {{questions}} WHERE parent_qid='$flt[0]' AND language='{$language}' AND scale_id=1 ORDER BY question_order";
		                ...
		                foreach($fresult->readAll() as $frow)
		                {
		                    ...
		                    if (isset($summary) && array_search($myfield2, $summary)!== FALSE) {$statisticsoutput .= " checked='checked'";}
		                    $statisticsoutput .= " />&nbsp;<strong>"
		                    .self::_showSpeaker($niceqtext." ".str_replace("'", "`", $row[1]." [".$frow['question']."]")." - ".$row[0]."/".$frow['title'])
		                    ."</strong><br />\n";
		        ...
		        case "H": // ARRAY (By Column)
		            ...
		            foreach($result->readAll() as $row)
		            {
		                 ...
		                 * - language
		                 */
		                $fquery = "SELECT * FROM {{answers}} WHERE qid={$flt[0]} AND language='{$language}' ORDER BY sortorder, code";
		                $fresult = Yii::app()->db->createCommand($fquery)->query();
		        ...
		        case "1": // MULTI SCALE
		            ...
		            foreach($result->readAll() as $row)
		            {
		                ...
		                $dshquery = "SELECT value FROM {{question_attributes}} WHERE qid={$flt[0]} AND attribute='dualscale_headerA'";
		                $dshresult = Yii::app()->db->createCommand($dshquery)->query();

		                //get header
		                foreach($dshresult->readAll() as $dshrow)
		                {
		                ...
		                $dshquery2 = "SELECT value FROM {{question_attributes}} WHERE qid={$flt[0]} AND attribute='dualscale_headerB'";
		                $dshresult2 = Yii::app()->db->createCommand($dshquery2)->query();

		                //get header
		                foreach($dshresult2->readAll() as $dshrow2)
		                {
