...
class surveypermission extends Survey_Common_Action {
    ...
    */
    function adduser($surveyid)
    {
        ...
        if($action == "addsurveysecurity")
        {
            ...
            if( hasSurveyPermission($surveyid, 'surveysecurity', 'create') && in_array($postuserid,getUserList('onlyuidarray')))
            {
    ...
    */
    function set($surveyid)
    {
        ...
        {
            if ( (!User::GetUserRights('superadmin') && Yii::app()->user->getId()==$postuserid) // User can not change own security (except superadmin)
        ...
        {
            if ( !in_array($postusergroupid,getUserList('onlyuidarray')) ) // User can not change own security (except for superadmin ?)
            {
                $this->getController()->error('Access denied');
            }
        }
