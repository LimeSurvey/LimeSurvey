...
class surveypermission extends Survey_Common_Action {
    ...
    */
    function index($surveyid)
    {
        ...
        if(App()->user->checkAccess('surveysecurity', ['crud' => 'read', 'entity' => 'survey', 'entity_id' => $surveyid]))
        {
            ...
            $surveysecurity ="<div class='header ui-widget-header'>".gT("Survey permissions")."</div>\n";
            ...
            if(count($result2) > 0)
            {
                ...
                foreach ($result2 as $PermissionRow)
                {
                    if(in_array($PermissionRow['uid'],$userList))
                    {
                        ...
                        if(App()->user->checkAccess('surveysecurity', ['crud' => 'update', 'entity' => 'survey', 'entity_id' => $surveyid]))
                        {
                        ...
                        if(App()->user->checkAccess('surveysecurity', ['crud' => 'delete', 'entity' => 'survey', 'entity_id' => $surveyid]))
                        {
                        ...
                        foreach ($aBaseSurveyPermissions as $sPKey=>$aPDetails) {
                            ...
                            {
                                if ($sPDetailValue && App()->authManager->checkAccess($sPKey, $PermissionRow['uid'], ['crud' => $sPDetailKey, 'entity' => 'survey', 'entity_id' => $surveyid]) && !($sPKey=='survey' && $sPDetailKey=='read')) $iCount++;
            ...
            if(App()->user->checkAccess('surveysecurity', ['crud' => 'create', 'entity' => 'survey', 'entity_id' => $surveyid]))
            {
                $surveysecurity .= CHtml::form(array("admin/surveypermission/sa/adduser/surveyid/{$surveyid}"), 'post', array('class'=>"form44"))."<ul>\n"
                . "<li><label for='uidselect'>".gT("User").": </label><select id='uidselect' name='uid'>\n"
                ...
                . "</select>\n"
                . "<input style='width: 15em;' type='submit' value='".gT("Add user group")."' onclick=\"if (document.getElementById('ugidselect').value == -1) { alert('".gT("Please select a user group first","js")."'); return false;}\" />"
    ...
    */
    function addusergroup($surveyid)
    {
        ...
        {
            $addsummary = "<div class=\"header\">".gT("Add user group")."</div>\n";
            $addsummary .= "<div class=\"messagebox ui-corner-all\" >\n";

            $result = Survey::model()->findAll('sid = :surveyid AND owner_id = :owner_id',array(':surveyid' => $surveyid, ':owner_id' => App()->user->id));
            if( App()->user->checkAccess('surveysecurity', ['crud' => 'create', 'entity' => 'survey', 'entity_id' => $surveyid])
                ...
                )
            {
                if($postusergroupid > 0){
                    ...
                    if(count($result2) > 0)
                    {
                        ...
                        if($isrresult)
                        {
                            $addsummary .= "<div class=\"successheader\">".gT("User group added.")."</div>\n";
                            Yii::app()->session['uids'] = $uid_arr;
                            $addsummary .= "<br />"
                            .CHtml::form(array("admin/surveypermission/sa/set/surveyid/{$surveyid}"), 'post')
                            ."<input type='submit' value='".gT("Set Survey Rights")."' />"
                        ...
                        else
                        {
                            ...
                            $addsummary .= "<div class=\"warningheader\">".gT("Failed to add user group.")."</div>\n";
                ...
                {
                    $addsummary .= "<div class=\"warningheader\">".gT("Failed to add user.")."</div>\n"
                    . "<br />" . gT("No Username selected.")."<br />\n";
                    $addsummary .= "<br/><input type=\"submit\" onclick=\"window.open('".$this->getController()->createUrl('admin/surveypermission/sa/view/surveyid/'.$surveyid)."', '_top')\" value=\"".gT("Continue")."\"/>\n";
                }
            }
            else
    ...
    */
    function adduser($surveyid)
    {
        ...
        {
            $addsummary = "<div class='header ui-widget-header'>".gT("Add user")."</div>\n";
            $addsummary .= "<div class=\"messagebox ui-corner-all\">\n";

            $result = Survey::model()->findAll('sid = :sid AND owner_id = :owner_id AND owner_id != :postuserid',array(':sid' => $surveyid, ':owner_id' => App()->user->id, ':postuserid' => $postuserid));
            if( App()->user->checkAccess('surveysecurity', ['crud' => 'create', 'entity' => 'survey', 'entity_id' => $surveyid])
                ...
                )
            {
                ...
                if($postuserid > 0){
                    ...
                    if($isrresult)
                    {
                        ...
                        $addsummary .= "<div class=\"successheader\">".gT("User added.")."</div>\n";
                        $addsummary .= "<br />"
                        .CHtml::form(array("admin/surveypermission/sa/set/surveyid/{$surveyid}"), 'post')
                        ."<input type='submit' value='".gT("Set survey permissions")."' />"
                        ."<input type='hidden' name='action' value='setsurveysecurity' />"
                    ...
                    else
                    {
                        ...
                        $addsummary .= "<div class=\"warningheader\">".gT("Failed to add user.")."</div>\n"
                        . "<br />" . gT("Username already exists.")."<br />\n";
                        $addsummary .= "<br/><input type=\"submit\" onclick=\"window.open('".$this->getController()->createUrl('admin/surveypermission/sa/view/surveyid/'.$surveyid)."', '_top')\" value=\"".gT("Continue")."\"/>\n";
                    }
                }
                else
    ...
    function set($surveyid)
    {
        ...
        if( App()->user->checkAccess('surveysecurity', ['crud' => 'update', 'entity' => 'survey', 'entity_id' => $surveyid]) )
        {
            ...
            if ($action == "setsurveysecurity")
            {
                ...
                $usersummary = "<div class='header ui-widget-header'>".sprintf(gT("Edit survey permissions for user %s"),"<span style='font-style:italic'>".$sUsername."</span>")."</div>";
            }
            else
            {
                $resrow = UserGroup::model()->find('ugid = :ugid',array(':ugid' => $postusergroupid));
                $sUsergroupName=$resrow['name'];
                $usersummary = "<div class='header ui-widget-header'>".sprintf(gT("Edit survey permissions for group %s"),"<span style='font-style:italic'>".$sUsergroupName."</span>")."</div>";
            }
            $usersummary .= "<br />"
                ...
                foreach ($aCRUDPermissions as $sCRUDKey=>$CRUDValue)
                {
                    ...
                    if ($CRUDValue)
                    {
                        if (!($sPermissionKey=='survey' && $sCRUDKey=='read'))
                        {
                            ...
                            if($action=='setsurveysecurity' && App()->authManager->checkAccess($sPermissionKey, $postuserid, ['crud' => $sCRUDKey, 'entity' => 'survey', 'entity_id' => $surveyid])) {
    ...
    */
    function surveyright($surveyid)
    {
        ...
        if ($action == "surveyrights" && App()->user->checkAccess('surveysecurity', ['crud' => 'update', 'entity' => 'survey', 'entity_id' => $surveyid]))
        {
            ...
            if($postuserid){
                if (!App()->user->checkAccess('superadmin'))
                {
                    ...
                    $resrow = Survey::model()->find($where,array(':surveyid' => $surveyid, ':owner_id' => App()->user->id, ':postuserid' => $postuserid));
                }
            }
            else{
                $where .= "sid = :sid";
            ...
            {
                $oResult = UserInGroup::model()->findAll('ugid = :ugid AND uid <> :uid AND uid <> :iOwnerID',array(':ugid' => $postusergroupid, ':uid' => App()->user->id, ':iOwnerID' => $iOwnerID));
                if(count($oResult) > 0)
                {
