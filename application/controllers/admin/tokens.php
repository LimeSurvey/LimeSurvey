...
class tokens extends Survey_Common_Action
{
    ...
    public function edit($iSurveyId, $iTokenId, $ajax = false)
    {
        ...
        if ($request->getPost('subaction')) {
            ...
            $udresult = Token::model($iSurveyId)->findAll("tid <> :tid and token <> '' and token = :token", [':tid' => $iTokenId, ':token' => Token::sanitizeToken($request->getPost('token'))]);
            $sOutput = '';
            if (count($udresult) == 0) {
                $thissurvey = getSurveyInfo($iSurveyId);
    ...
    public function addDummies($iSurveyId, $subaction = '')
    {
        ...
        if (!empty($subaction) && $subaction == 'add') {
            ...
            $aData['token'] = '';
            $aData['language'] = Yii::app()->request->getPost('language');
            $aData['sent'] = 'N';
...
// Validate sUploadCharset
            ...
            LSUploadHelper::checkUploadedFileSizeAndRedirect('the_file', App()->createUrl('admin/tokens', array('sa' => 'import', 'surveyid' => $iSurveyId)));
