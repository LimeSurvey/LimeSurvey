...
class index extends CAction
{
    ...
    public function _getParameters($args = array(), $post = array())
    {
        $param = array();
        if (@$args[0] == __CLASS__) {
            array_shift($args);
    ...
    public function _loadRequiredHelpersAndLibraries()
    {
    ...
    public function _loadLimesurveyLang($mvSurveyIdOrBaseLang)
    {
    ...
    public function _isClientTokenDifferentFromSessionToken($clientToken, $surveyid)
    {
        return $clientToken != '' && isset($_SESSION['survey_'.$surveyid]['token']) && $clientToken != $_SESSION['survey_'.$surveyid]['token'];
    }

    public function _isSurveyFinished($surveyid)
    {
        return isset($_SESSION['survey_'.$surveyid]['finished']) && $_SESSION['survey_'.$surveyid]['finished'] === true;
    }

    public function _surveyCantBeViewedWithCurrentPreviewAccess($surveyid, $bIsSurveyActive, $bSurveyExists)
    {
    ...
    public function _didSessionTimeout($surveyid)
    {
    ...
    public function _niceExit(&$redata, $iDebugLine, $sTemplateDir = null, $asMessage = array())
    {
    ...
    public function _createNewUserSessionAndRedirect($surveyid, &$redata, $iDebugLine, $asMessage = array())
    {
        killSurveySession($surveyid);
        $thissurvey = getSurveyInfo($surveyid);
        if ($thissurvey) {
    ...
    public function _printMessage($asLines)
    {
        if (func_num_args() > 1) {
    ...
    public function _printTemplateContent($sTemplateFile, &$redata, $iDebugLine = -1)
    {
