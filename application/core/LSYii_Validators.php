...
class LSYii_Validators extends CValidator
{
    ...
    public function fixCKeditor($value)
    {
        ...
        if (trim($value) == '&nbsp;' || trim($value) == '') { // chrome adds a single &nbsp; element to empty fckeditor fields
            $value = '';
        }
    ...
    public function xssFilter($value)
    {
        ...
        foreach ($aValues as $key => $aValue) {
            if ($aValue[2] == 'STRING') {
            ...
            }// If EM is broken : can throw invalid $key
            else {
                ...
                foreach ($aParsedExpressions as $aParsedExpression) {
                    ...
                    } // This disallow complex HTML construction with XSS
