...
class CintLinkOrder {
  ...
  const selfRel = "http://cds.cint.com/rel/self";
  const releaseRel = "http://cds.cint.com/rel/order/transition/release";
  const cancelRel = "http://cds.cint.com/rel/order/transition/cancel";

  public function __construct($orderXml, $context) {
    $this->construct($orderXml);
    $this->context = $context;
  }

  private function construct($orderXml) {
    $this->url = CintLinkAPI::getLink(CintLinkOrder::selfRel, $orderXml->link);
    $this->raw = $orderXml;
  }

  public function release() {
    $releaseUrl = CintLinkAPI::getLink(CintLinkOrder::releaseRel, $this->raw->link);
    if ($releaseUrl) {
      $curl = $this->context->setupAuthorizedRequest($orderUrl);
      $resp = $curl->put($orderUrl);

      if ($resp->statusCode < 200 || $resp->statusCode >= 300)
        throw new Exception($resp->body, $resp->statusCode);
      $this->construct($context->getOrder($this->url)->getRaw());
      return $this;
    }
    return false;
  }
