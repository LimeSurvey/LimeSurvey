...
class SettingsWidget extends CWidget {
  ...
  public function init() {
    ...
    Yii::app()->getClientScript()->registerCssFile(App()->getAssetManager()
      ->publish(dirname(__FILE__) . '/assets/settingswidget.css'));
    Yii::app()->getClientScript()->registerScriptFile(App()->getAssetManager()
      ->publish(dirname(__FILE__) . '/assets/settingswidget.js'));
  ...
  protected function renderSetting($name, $metaData, $form = NULL, $return = FALSE) {
    ...
    if (isset($metaData['type'])) {
      ...
      $result = CHtml::tag('div', array(
        'class' => 'setting',
        'data-name' => $name
      ), $this->$function($name, $metaData, $form)); // render inside a div
  ...
  public function renderCheckbox($name, array $metaData, $form = NULL) {
    ...
    $out .= CHtml::checkBox($id, $value, array(
      'id' => $id,
      'form' => $form,
      'container' => 'div',
      'separator' => ''
  ...
  public function renderRelevance($name, array $metaData, $form = NULL) {
    ...
    $out .= CHtml::textArea($name, $value, array(
      'id' => $id,
      'form' => $form,
  ...
  public function renderSelect($name, array $metaData, $form = NULL) {
    ...
    $out .= App()->getController()
      ->widget('ext.bootstrap.widgets.TbSelect2', $properties, TRUE);
    // add a description to form fields - html-tag are possible
    (isset ($metaData['description'])) ? $out .= CHtml::tag('div', array('class' => 'description help-block'), $metaData['description'], TRUE) : '';
    return $out;
  }
  ...
  public function renderText($name, array $metaData, $form = NULL) {
    ...
    $out .= CHtml::textArea($id, $value, array(
      'id' => $id,
      'form' => $form,
      'readonly' => $readOnly,
  ...
  public function renderPassword($name, array $metaData, $form = NULL) {
    ...
    $out .= CHtml::passwordField($id, $value, array(
      'id' => $id,
      'form' => $form
    ));
    // add a description to form fields - html-tag are possible
  ...
  public function renderList($name, array $metaData, $form = NULL) {
    ...
    $cells .= CHtml::tag('td', array(), $this->widget('bootstrap.widgets.TbButtonGroup', array(
      ...
      'buttons' => array(
        array(
          'icon' => 'icon-minus',
          'htmlOptions' => array('class' => 'remove')
        ),
    ...
    // add a description to form fields - html-tag are possible
    (isset ($metaData['description'])) ? $result .= CHtml::tag('div', array('class' => 'description help-block'), $metaData['description'], TRUE) : '';
    return $result;
  }
}
