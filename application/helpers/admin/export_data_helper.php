...
function spss_export_data ($na = null) {
    ...
    $result=db_execute_num($query) or safe_die("Couldn't get results<br />$query"); //Checked
...
function spss_getvalues ($field = array(), $qidattributes = null ) {
    ...
    if (strpos("!LORFWZWH1",$field['LStype']) !== false) {
        if (substr($field['code'],-5) == 'other' || substr($field['code'],-7) == 'comment') {
            ...
            $result=$CI->questions_model->valueLabel($field,$language); //db_execute_assoc($query) or safe_die("Couldn't lookup value labels<br />$query<br />".$connect->ErrorMsg()); //Checked
...
function spss_fieldmap($prefix = 'V') {
    global $surveyid, $typeMap;//, $connect, $clang;
    ...
    $field = array('sid','anonymized', 'language');
    //$query="SELECT sid, anonymized, language FROM {$dbprefix}surveys WHERE sid=$surveyid";
    $CI->load->model('surveys_model');
    $result=$CI->surveys_model->getSomeRecords($field,$condition);//($query) or safe_die("Couldn't count fields<br />$query<br />".$connect->ErrorMsg());  //Checked)
    $num_results = $result->num_rows();
    ...
    $tempArray = array();
    $fieldnames = $CI->db->list_fields();//($connect->MetaColumnNames("{$dbprefix}survey_$surveyid", true));
...
function spss_getquery() {
    ...
    if (isset($tokensexist) && $tokensexist == true && $surveyprivate == 'N') {
        ...
        foreach ($tokenattributes as $attributefield=>$attributedescription) {
            //Drop the token field, since it is in the survey too
            if($attributefield!='token') {
                $query .= $CI->db->dbprefix."tokens_$surveyid.$attributefield, ";
            }
        }
        $query .= $CI->db->dbprefix."survey_$surveyid.*
	    FROM ".$CI->db->dbprefix."survey_$surveyid
	    LEFT JOIN ".$CI->db->dbprefix."tokens_$surveyid ON ".$CI->db->dbprefix."survey_$surveyid.token = ".$CI->db->dbprefix."tokens_$surveyid.token";
    } else {
        $query = "SELECT *
	    FROM ".$CI->db->dbprefix."survey_$surveyid";
    }
...
function BuildXMLFromQuery($xmlwriter, $Query, $tagname='', $excludes = array())
{
    ...
    $QueryResult = db_execute_assoc($Query) or safe_die ("ERROR: $QueryResult"); //safe
    ...
    else
    {
        ...
        $TableName = substr($TableName, strlen($CI->db->dbprefix), strlen($TableName));
    }
    if ($QueryResult->num_rows()>0)
    {
        ...
        $Columninfo =  $QueryResult->list_fields();
        ...
        while ($Row = $QueryResult->result_array())
        {
