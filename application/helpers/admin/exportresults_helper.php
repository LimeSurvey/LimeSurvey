...
class SurveyDao
{
    ...
    */
    public function loadSurveyById($id)
    {
        ...
        //Load tokens
        if (tableExists('{{tokens_' . $intId . '}}') && hasSurveyPermission($intId,'token','read'))
        {
    ...
    */
    public function loadSurveyResults(SurveyObj $survey, $iLimit, $iOffset, $iMaximum, $sFilter='' )
    {
        ...
        if (tableExists('tokens_'.$survey->id) && array_key_exists ('token',Survey_dynamic::model($survey->id)->attributes) && hasSurveyPermission($survey->id,'token','read'))
        {
