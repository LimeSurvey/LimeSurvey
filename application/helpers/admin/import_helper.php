        ...
        while ($oXMLReader->read()) {
            if ($oXMLReader->name === 'LimeSurveyDocType' && $oXMLReader->nodeType == XMLReader::ELEMENT) {
                ...
                if ($oXMLReader->value != 'Responses') {
                    $results['warnings'][] = gT("This is not a valid response data XML file.");
                    return $results;
                }
            }
                    ...
                    if ($oXMLReader->name === 'row' && $oXMLReader->nodeType == XMLReader::ELEMENT) {
                        ...
                        try {
                            ...
                            if (!$response->encryptSave(true)) {
                                $results['warnings'][] =  CHtml::errorSummary($response, '<b>' . sprintf(gT('Table name: %s'), gT('Responses', 'unescaped')) . '</b>');
                                return $results;
                            }
                        } catch (Exception $e) {
