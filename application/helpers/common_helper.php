...
function getGroupList($gid, $surveyid)
{
    ...
    if ($groupselecter) {
        if (!isset($gvexist)) {
            ...
            $groupselecter .= "<option value='" . Yii::app()->getConfig('scriptname') . "?sid=$surveyid&amp;gid='>" . gT("None") . "</option>\n";
        }
    }
...
function getGroupListLang($gid, $language, $surveyid)
{
    ...
    $gidresult = QuestionGroup::model()->findAll(array(
        'condition' => 'sid=:surveyid AND language=:language',
        'order' => 'group_order',
        'params' => array(':surveyid' => $surveyid, ':language' => $language)
    )); //Checked)
    foreach ($gidresult as $gv) {
        $gv = $gv->attributes;
        $groupselecter .= "<option";
...
function getIPAddress()
{
    ...
    if (!empty($_SERVER['HTTP_CLIENT_IP']) && filter_var($_SERVER['HTTP_CLIENT_IP'], FILTER_VALIDATE_IP) !== false) {
        ...
        // IP Address forwarded from proxy
        ...
        if (false === $vComma && filter_var($_SERVER['HTTP_X_FORWARDED_FOR'], FILTER_VALIDATE_IP) !== false) {
            ...
            // IP Address forwarded from multiple proxies
