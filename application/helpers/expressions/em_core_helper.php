...
class ExpressionManager
{
    ...
    public function __construct()
    {
        ...
        $RDP_regex_assign = '=';    // '=|\+=|-=|\*=|/=';
        $RDP_regex_sgqa = '(?:INSERTANS:)?[0-9]+X[0-9]+X[0-9]+[A-Z0-9_]*\#?[01]?(?:\.(?:'.self::$RDP_regex_var_attr.'))?';
        $RDP_regex_word = '(?:TOKEN:)?(?:[A-Z][A-Z0-9_]*)?(?:\.(?:[A-Z][A-Z0-9_]*))*(?:\.(?:'.self::$RDP_regex_var_attr.'))?';
...
'unique' => array('exprmgr_unique', 'LEMunique', gT('Returns true if all non-empty responses are unique'), 'boolean unique(arg1, ..., argN)', '', -1),
    ...
    private function RDP_AddError($errMsg, $token)
    {
      ...
      * @param array $token
      *
      * @return bool - false if there is any error, else true
      */
     ...
     * @param array $token
     *
     * @return bool - true if success, false if any error occurred
     */
...
//                    $resolvedParts[] = "<span id='" . $idName . "'>" . htmlspecialchars($resolvedPart,ENT_QUOTES,'UTF-8',false) . "</span>"; // TODO - encode within SPAN?
     ...
     * @param type $eqn
     */
    public function ExpandThisVar($src)
    {
     ...
     * @return string
     */
    public static function ShowAllowableFunctions()
    {
        $em = new self();
...
function exprmgr_countifop($args)
{
    ...
    foreach ($args as $arg) {
        switch ($op) {
            case '==':  case 'eq': if ($arg == $value) {
     ++$j;
 } break;
            case '>=':  case 'ge': if ($arg >= $value) {
     ++$j;
 } break;
            case '>':   case 'gt': if ($arg > $value) {
     ++$j;
 } break;
            case '<=':  case 'le': if ($arg <= $value) {
     ++$j;
 } break;
            case '<':   case 'lt': if ($arg < $value) {
     ++$j;
 } break;
            case '!=':  case 'ne': if ($arg != $value) {
     ++$j;
 } break;
            case 'RX':
                try {
