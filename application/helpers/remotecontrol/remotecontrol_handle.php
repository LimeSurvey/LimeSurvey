...
class remotecontrol_handle
{
    ...
    */
    public function activate_survey($sSessionKey, $iSurveyID)
    {
        if ($this->_checkSessionKey($sSessionKey))
        {
            ...
            if (App()->user->checkAccess('surveyactivation', ['crud' => 'update', 'entity' => 'survey', 'entity_id' => $iSurveyID]))
            {
    ...
    */
    public function export_statistics($sSessionKey, $iSurveyID,  $docType='pdf', $sLanguage=null, $graph='0', $groupIDs=null)
    {
        ...
        if (!App()->user->checkAccess('statistics', ['crud' => 'read', 'entity' => 'survey', 'entity_id' => $iSurveyID]))
    ...
    */
    public function get_group_properties($sSessionKey, $iGroupID, $aGroupSettings)
    {
        if ($this->_checkSessionKey($sSessionKey))
        {
            ...
            if (App()->user->checkAccess('survey', ['crud' => 'read', 'entity' => 'survey', 'entity_id' => $oGroup->sid]))
            {
    ...
    * @return array The list of tokens    * @param array $aConditions Conditions to limit the list, e.g. with array(1 => 'email = "info@example.com"')
