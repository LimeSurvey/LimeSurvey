    ...
    try {
        ...
        */
        ...
        if ($iOldDBVersion < 400) {
            ...
            $floatDBVersion = Yii::app()->getConfig('DBVersion');
            /* 359 to 399.1 : Question table */
            if ($floatDBVersion < 399.1) {
                $oTransaction = $oDB->beginTransaction();
                /*Prepare : remove previous try. keep _update400 DB if exist */
                if(!Yii::app()->db->schema->getTable('{{questions_update400}}')){
                    $oDB->createCommand()->renameTable('{{questions}}', '{{questions_update400}}');
                }
                if(Yii::app()->db->schema->getTable('{{questions}}')){
                    $oDB->createCommand()->dropTable('{{questions}}');
                }
                ...
                $oDB->createCommand("INSERT INTO {{question_l10ns}} (qid, question, help, language) select qid, question, help, language from {{questions_update400}}")->execute();
                ...
                /* Drop the index for pgsql (to confirm) */
                $oDB->createCommand()->dropIndex('{{idx1_questions}}', '{{questions_update400}}');
                $oDB->createCommand()->dropIndex('{{idx2_questions}}', '{{questions_update400}}');
                $oDB->createCommand()->dropIndex('{{idx3_questions}}', '{{questions_update400}}');
                $oDB->createCommand()->dropIndex('{{idx4_questions}}', '{{questions_update400}}');
                $oDB->createCommand()->dropIndex('{{idx5_questions}}', '{{questions_update400}}');
                ...
                $oDB->createCommand()->update('{{settings_global}}', array('stg_value'=>399.1), "stg_name='DBVersion'");
                /* Final : set the DBVersion and drop backup table */
                Yii::app()->setConfig('DBVersion',399.1);
                $floatDBVersion= 399.1;
                $oTransaction->commit();
                $oDB->createCommand()->dropTable('{{questions_update400}}');
            }
            /* 399.1 to 399.2 : Group table */
            if ($floatDBVersion < 399.2) {
                /*Prepare : remove previous try. keep _update400 DB if exist */
                if(!Yii::app()->db->schema->getTable('{{groups_update400}}')){
                    $oDB->createCommand()->renameTable('{{groups}}', '{{groups_update400}}');
                }
                if(Yii::app()->db->schema->getTable('{{groups}}')){
                    $oDB->createCommand()->dropTable('{{groups}}');
                }
                ...
                $oDB->createCommand("INSERT INTO {{group_l10ns}} (gid, group_name, description, language) select gid, group_name, description, language from {{groups_update400}}")->execute();
                /* Net group table */
                ...
                /* The index */
                $oDB->createCommand()->dropIndex('{{idx1_groups}}', '{{groups_update400}}');
                $oDB->createCommand()->createIndex('{{idx1_groups}}', '{{groups}}', 'sid', false);
                /* Final : set the DBVersion and drop backup table */
                Yii::app()->setConfig('DBVersion',399.2);
                $floatDBVersion= 399.2;
                $oTransaction->commit();
                $oDB->createCommand()->dropTable('{{groups_update400}}');
            }
            /* 399.2 to 399.3 : Answer table */
            if ($floatDBVersion < 399.3) {
                /*Prepare : remove previous try. keep _update400 DB if exist */
                if(!Yii::app()->db->schema->getTable('{{answers_update400}}')){
                    $oDB->createCommand()->renameTable('{{answers}}', '{{answers_update400}}');
                }
                if(Yii::app()->db->schema->getTable('{{answers}}')){
                    $oDB->createCommand()->dropTable('{{answers}}');
                }
                ...
                ")->execute();
                /* The index */
                $oDB->createCommand()->dropIndex('{{answers_idx}}', '{{answers_update400}}');
                $oDB->createCommand()->dropIndex('{{answers_idx2}}', '{{answers_update400}}');
                ...
                /* Final : set the DBVersion and drop backup table */
                Yii::app()->setConfig('DBVersion',399.3);
                $floatDBVersion= 399.3;
                $oTransaction->commit();
                $oDB->createCommand()->dropTable('{{answers_update400}}');
            }
            /* 399.3 to 399.4 : Label table */
            if ($floatDBVersion < 399.4) {
                /*Prepare : remove previous try. keep _update400 DB if exist */
                if(!Yii::app()->db->schema->getTable('{{labels_update400}}')){
                    $oDB->createCommand()->renameTable('{{answers}}', '{{labels_update400}}');
                }
                if(Yii::app()->db->schema->getTable('{{labels}}')){
                    $oDB->createCommand()->dropTable('{{labels}}');
                }
                ...
                $oDB->createCommand("INSERT INTO {{label_l10ns}}
                    ...
                    ")->execute();
                /* Final : set the DBVersion and drop backup table */
                Yii::app()->setConfig('DBVersion',399.4);
                $floatDBVersion= 399.4;
                $oTransaction->commit();
