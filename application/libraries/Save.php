...
class Save
{
    ...
    public function savedcontrol()
    {
        ...
        if (empty($_POST['savename'])) {
            $errormsg .= gT('You must supply a name for this saved session.')."<br />\n";
        }
        if (empty($_POST['savepass'])) {
            $errormsg .= gT('You must supply a password for this saved session.')."<br />\n";
        }
     ...
     */
    public function set_answer_time()
    {
        global $thissurvey;
