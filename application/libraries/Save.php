...
class Save
{
    ...
    function saveSurvey()
    {
        ...
        if (empty($this->aSaveErrors)) {
            ...
            $_SESSION['reponses_' . $surveyid]['holdname'] = $this->saveData['identifier']; //Session variable used to load answers every page. Unsafe - so it has to be taken care of on output
            $_SESSION['response_' . $surveyid]['holdpass'] = $this->saveData['clearpassword']; //Session variable used to load answers every page. Unsafe - so it has to be taken care of on output
     ...
     * The times are saved in table: {prefix}_timings_{sid}
     * @return void
     */
