...
class Participant extends LSActiveRecord
{
    ...
    public function deleteParticipants($rows, $bFilter = true)
    {
        ...
        foreach ($aParticipantsIDChunks as $aParticipantsIDs) {
            ...
            foreach ($aParticipantsIDs as $aID) {
                $oParticipant = self::model()->findByPk($aID);
                if ($oParticipant) {
                    $oParticipant->delete();
                }
            }

            Yii::app()->db->createCommand()->delete(self::model()->tableName(), array('in', 'participant_id', $aParticipantsIDs));
    ...
    public function filterParticipantIDs($aParticipantIDs)
    {
        if (!Permission::model()->hasGlobalPermission('superadmin', 'read')) {
            ...
            $aParticipantIDs = Yii::app()->db->createCommand()->select('participant_id')->from(self::model()->tableName())->where($aCondition, $aParameter)->queryColumn();
        }
            ...
            foreach ($tokens as $key => $value) {
                ...
                if (Yii::app()->db->schema->getTable($tokentable)) {
                    ...
                    if ($datas = Yii::app()->db->schema->getTable($surveytable)) {
                        if (!empty($token['token']) && isset($datas->columns['token']) && Permission::model()->hasSurveyPermission($iSurveyID, 'responses', 'delete')) {
    ...
    public function getParticipantsSearchMultiple($condition, $page, $limit)
    {
        ...
        if ($page == 0 && $limit == 0) {
            $arr = self::model()->findAll($command);
            $data = array();
            foreach ($arr as $t) {
...
//                $command->addCondition('(attribute_id='. $sFieldname . ' AND value ' . $operator . ' '.$param.' )', $booloperator);
     ...
     * */
    public function copyCPBDAttributesToTokens($surveyid, $mapped, $newcreate, $participantid, $overwriteauto = false, $overwriteman = false, $overwritest = false, $createautomap = true)
    {
        ...
        if (!empty($newcreate)) {
            //Create new fields in the tokens table
                ...
                if ($overwriteauto == 'true') {
                    ...
                    if (!empty($newcreate)) {
                        ...
                        for ($a = 0; $a < $numberofattributes; ++$a) {
                            self::model()->updateTokenAttributeValue($surveyid, $sParticipantUID, $attributesadded[$a], $attributeidadded[$a]);
                        }
                    }
                    ...
                    if (!empty($mapped)) {
                        foreach ($mapped as $key => $value) {
                            if ($key[10] == 'c') { //We know it's automapped because the 11th letter is 'c'
                                self::model()->updateTokenAttributeValue($surveyid, $sParticipantUID, $value, $key);
                            }
                        }
                    }
                }
        ...
        foreach ($tokenid as $key => $tid) {
            if (is_numeric($tid) && $tid != '') {
                ...
                if (count($query) > 0) {
                    ...
                    if ($overwriteman == 'true' && !empty($aMapped)) {
                        foreach ($aMapped as $cpdbatt => $tatt) {
                            self::model()->updateAttributeValueToken($surveyid, $query[0]['participant_id'], $cpdbatt, $tatt);
                        }
                    }
                }
                /* If there isn't an existing entry, create one! */
                    ...
                    if (!empty($aAttributesToBeCreated)) {
                        foreach ($aAttributesToBeCreated as $key => $value) {
                            self::model()->updateAttributeValueToken($surveyid, $pid, $attid[$key], $key);
                        }
                    }
                    /* Now add mapped attribute values */
                    if (!empty($aMapped)) {
                        foreach ($aMapped as $cpdbatt => $tatt) {
                            self::model()->updateAttributeValueToken($surveyid, $pid, $cpdbatt, $tatt);
                        }
                    }
                    ++$sucessfull;
