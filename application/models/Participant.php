...
class Participant extends LSActiveRecord
{
    ...
    /**
     * This function deletes the participant from the participants list,
    ...
    /**
     * Deletes CPDB participants identified by their participant ID from survey participants lists
     *
    ...
    private function writeParticipantsToTokenTable(
        ...
        foreach ($oParticipants as $oParticipant) {
            ...
            if ($isDuplicate) {
                //Participant already exists in survey participants list - don't copy
                $duplicate++;
    ...
    /**
     * Copies central attributes/participants to an individual survey survey participants list
     *
    ...
    public function copyCPDBAttributesToTokens($surveyId, array $participantIds, array $mappedAttributes, array $newAttributes, array $options)
    {
        ...
        //Write each participant to the survey survey participants list
    ...
    /**
     * Updates a field in the survey participants list with a value from the participant attributes table
     *
     ...
     * @param int $tokenFieldname fieldname in the survey participants list
     *
    ...
    /**
     * Updates or creates a field in the survey participants list with a value from the participant attributes table
     *
     * @param int $surveyId Survey ID number
    ...
    /**
     * Copies token participants to the central participants list, and also copies
    ...
    {
        $survey = Survey::model()->findByPk($surveyid);
        $tokenid_string = Yii::app()->session['participantid']; //List of token_id's to add to participants list
        ...
        if (!empty($aAttributesToBeCreated)) {
            foreach ($aAttributesToBeCreated as $key => $value) {
                ...
                /* $key is the fieldname from the survey participants list (ie "attribute_1")
        ...
        foreach ($tokenids as $tid) {
            if (is_numeric($tid) && $tid != "") {
                ...
                if ($existing != null) {
                    ...
                    //Update survey participants list and insert the new UUID
