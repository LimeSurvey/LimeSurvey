...
class ParticipantAttributeName extends LSActiveRecord
{
    ...
    public function getVisibleAttributes($sLanguageFilter = null)
    {
        ...
        $ids = self::model()->findAll("visible = 'TRUE'");
    ...
    public function getParticipantVisibleAttribute($participant_id)
    {
        ...
        if ($participant_id != '') {
            ...
            $records = self::model()->with('participant_attribute_names_lang', 'participant_attribute')
            ...
            $findCriteria = new CDbCriteria();
            $records = self::model()->with('participant_attribute_names_lang', 'participant_attribute')->findAll($findCriteria);
            foreach ($records as $row) { //Iterate through each attribute
                $thisname = '';
                $thislang = '';
                foreach ($row->participant_attribute_names_lang as $names) { //Iterate through each language version of this attribute
    ...
    public function getCPDBAttributes($count = false, $limit = -1, $offset = -1)
    {
        $findCriteria = new CDbCriteria();
        $findCriteria->offset = $offset;
        $findCriteria->limit = $limit;
        $output = array();
        $records = self::model()->with('participant_attribute_names_lang')->findAll($findCriteria);
        foreach ($records as $row) { //Iterate through each attribute
            $thisname = '';
            $thislang = '';
            foreach ($row->participant_attribute_names_lang as $names) { //Iterate through each language version of this attribute
    ...
    public function getAttributesValues($attribute_id)
    {
    ...
    public function getnotaddedAttributes($attributeid)
    {
        $output = array();
        $notin = array();
        ...
        $records = self::model()->with('participant_attribute_names_lang')->findAll($criteria);
        foreach ($records as $row) { //Iterate through each attribute
            $thisname = '';
            $thislang = '';
            foreach ($row->participant_attribute_names_lang as $names) { //Iterate through each language version of this attribute
    ...
    public function storeAttribute($data)
    {
        ...
        if (self::model()->count() > 59) {
            return false;
        };
        $oParticipantAttributeName = new self();
    ...
    public function saveAttribute($data)
    {
        ...
        if (!empty($insertnames)) {
            $oParticipantAttributeName = self::model()->findByPk($data['attribute_id']);
