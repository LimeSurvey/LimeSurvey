...
class Permission extends LSActiveRecord
{
    ...
    public static function getPermissions($iUserID, $iEntityID = null, $sEntityName = null)
    {
        if ($sEntityName == 'survey') {
            $aBasePermissions = self::model()->getSurveyBasePermissions();
        } elseif ($sEntityName == 'global') {
            $aBasePermissions = self::model()->getGlobalBasePermissions();
        }

        if (is_null($sEntityName)) {
            $oPermissions = self::model()->findAllByAttributes(array('uid' => $iUserID));
            ...
            foreach ($aBasePermissions as $sPermission => &$aPermissionDetail) {
                $oCurrentPermissions = self::model()->findByAttributes(array('uid' => $iUserID, 'entity_id' => $iEntityID, 'permission' => $sPermission));
    ...
    public static function setPermissions($iUserID, $iEntityID, $sEntityName, $aPermissions, $bBypassCheck = false)
    {
        ...
        if ($sEntityName == 'global') {
            $aBasePermissions = self::model()->getGlobalBasePermissions();
            if (!self::model()->hasGlobalPermission('superadmin', 'read') && !$bBypassCheck) {
                ...
                foreach ($aBasePermissions as $PermissionName => $aPermission) {
                    foreach ($aPermission as $sPermissionKey => &$sPermissionValue) {
                        if ($sPermissionKey != 'title' && $sPermissionKey != 'img' && !self::model()->hasGlobalPermission($PermissionName, $sPermissionKey)) {
            ...
            } elseif (self::model()->hasGlobalPermission('superadmin', 'read') && Yii::app()->session['loginID'] != 1) {
        ...
        if (Yii::app()->session['loginID'] != 1) {
            self::model()->deleteAllByAttributes($condition, "permission <> 'superadmin' AND entity <> 'template'");
        } else {
            self::model()->deleteAllByAttributes($condition, "entity <> 'template'");
        }
