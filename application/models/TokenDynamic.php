...
class TokenDynamic extends LSActiveRecord
{
    ...
    public function findUninvited($aTokenIds = false, $iMaxEmails = 0, $bEmail = true, $SQLemailstatuscondition = '', $SQLremindercountcondition = '', $SQLreminderdelaycondition = '')
    {
        ...
        $oResult = self::model()->findAll($command);
     ...
     * @return \CDbCriteria
     */
    public function getSearchMultipleCondition($condition)
    {
        $i = 0;
        $j = 1;
        $tobedonelater = array();
    ...
    {
        $dlquery = 'DELETE FROM '.self::tableName().' WHERE tid=:tokenid';
    ...
    public function deleteRecords($iTokenIds)
    {
        ...
        $dlquery = 'DELETE FROM '.self::tableName().' WHERE tid IN ('.implode(', ', $iTokenIds).')';
