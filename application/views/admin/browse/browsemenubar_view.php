...
<?php if (bHasSurveyPermission($surveyid,'responses','read')) { ?>
    ...
    <?php if (count(GetAdditionalLanguagesFromSurveyID($surveyid)) == 0) { ?>
        <a href='<?php echo $this->createUrl("admin/browse/surveyid/$surveyid/sa/all");?>' title="<?php echo $clang->gTview("Display Responses");?>">
        <img name='ViewAll' src='<?php echo $imageurl;?>/document.png' title='' alt='<?php echo $clang->gT("Display Responses");?>' /></a>
    <?php } else { ?>
        <a href="<?php echo $this->createUrl("admin/browse/surveyid/$surveyid/sa/all");?>" accesskey='b' id='browseresponses' title="<?php echo $clang->gTview("Display Responses");?>" >
    ...
    <?php } ?>
    <a href='<?php echo $this->createUrl("admin/browse/surveyid/$surveyid/sa/all/start/0/limit/50/order/desc");?>' title="<?php echo $clang->gTview("Display Last 50 Responses");?>" >
...
if (bHasSurveyPermission($surveyid,'responses','create')) { ?>
    <a href='<?php echo $this->createUrl("admin/dataentry/sa/view/surveyid/$surveyid");?>' title="<?php echo $clang->gTview("Dataentry Screen for Survey");?>" >
...
if (bHasSurveyPermission($surveyid,'responses','create'))
{ ?>
    <a href='<?php echo $this->createUrl("admin/dataentry/sa/import/surveyid/$surveyid");?>' title="<?php echo $clang->gTview("Import responses from a deactivated survey table");?>">
