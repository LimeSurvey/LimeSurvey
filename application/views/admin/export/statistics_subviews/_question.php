...
if (in_array($flt[2], [Question::QT_X_TEXT_DISPLAY,Question::QT_ASTERISK_EQUATION])) {
    return;
}
    ...
    <?php echo "<!-- Question type :  $flt[2] -->"; ?>
        <?php if ($flt[2] == Question::QT_M_MULTIPLE_CHOICE || $flt[2] == Question::QT_VERTICAL_FILE_UPLOAD || $flt[2] == Question::QT_P_MULTIPLE_CHOICE_WITH_COMMENTS || $flt[2] == Question::QT_L_LIST || $flt[2] == Question::QT_5_POINT_CHOICE || $flt[2] == Question::QT_G_GENDER || $flt[2] == Question::QT_I_LANGUAGE || $flt[2] == Question::QT_O_LIST_WITH_COMMENT || $flt[2] == Question::QT_Y_YES_NO_RADIO || $flt[2] == Question::QT_EXCLAMATION_LIST_DROPDOWN) : ?>
            ...
            <?php if ($flt[2] != Question::QT_N_NUMERICAL && $flt[2] != Question::QT_VERTICAL_FILE_UPLOAD) :?>
                <select name='<?php
                if ($flt[2] == Question::QT_M_MULTIPLE_CHOICE) {
                    echo Question::QT_M_MULTIPLE_CHOICE;
                };
                if ($flt[2] == Question::QT_P_MULTIPLE_CHOICE_WITH_COMMENTS) {
                    echo Question::QT_P_MULTIPLE_CHOICE_WITH_COMMENTS;
                };
        ...
        switch ($flt[2]) {
                ...
                foreach ($result[$key1] as $row1) {
                    ...
                    foreach ($row1 as $row) {
                        ...
                        */
                        ...
                        $myfield1 = "K" . $myfield . "_S" . $row['qid'];
                        $myfield2 = "K{$myfield}" . "_S" . $row['qid'] . "G";
                        $myfield3 = "K{$myfield}" . "_S" . $row['qid'] . "L";
                        if ($counter2 == 4) {
                            echo "\t</tr>\n\t<tr>\n";
                            $counter2 = 0;
                        }
                        ...
                        if (isset($summary) && (array_search("KQ{$flt[0]}_S{$row['qid']}", $summary) !== false)) {
                            echo " checked='checked'";
                        }
                        ...
                        //show speaker
                        echo $flt[3] . " - " . $oStatisticsHelper::_showSpeaker(flattenText($row['question'], true))
                        . "</div>
            ...
            case Question::QT_Q_MULTIPLE_SHORT_TEXT: // Multiple short text
                echo '<h4 class="question-selector-title">' . $oStatisticsHelper::_showSpeaker($niceqtext) . '</h4><br/>';
                //get subqestions
                $result[$key1] = Question::model()->getQuestionsForStatistics('title, question, qid', "parent_qid='$flt[0]'", 'question_order');
                //$counter2=0;
                ...
                foreach ($result[$key1] as $row) {
                    ...
                    $myfield2 = "Q" . $myfield . "_S" . $row['qid'];
                    echo "&nbsp;&nbsp; <input type='checkbox'  name='summary[]' value='$myfield2'";

                    if (isset($summary) && (array_search("QQ{$flt[0]}_S{$row['qid']}", $summary) !== false)) {
                        echo " checked='checked'";
                    }
            ...
            */
            case Question::QT_D_DATE: // Date
                ...
                */
                ...
                if (isset($summary) && (array_search("DQ{$flt[0]}", $summary) !== false)) {
                    echo " checked='checked'";
                }

                echo " />";
                echo $flt[3] . " - " . $oStatisticsHelper::_showSpeaker($niceqtext) . "
            ...
            case Question::QT_Y_YES_NO_RADIO: // Yes\No
                ...
                //pre-select values which were marked before
                if (isset($_POST[$myfield]) && is_array($_POST[$myfield]) && in_array("Y", $_POST[$myfield])) {
                    echo " selected='selected' ";
                }
                ...
                //pre-select values which were marked before
                if (isset($_POST[$myfield]) && is_array($_POST[$myfield]) && in_array("N", $_POST[$myfield])) {
                    echo " selected='selected' ";
                }
            ...
            case Question::QT_A_ARRAY_5_POINT: // Array of 5 point choice questions
                echo '<h4 class="question-selector-title">' . $oStatisticsHelper::_showSpeaker($niceqtext) . '</h4><br/>';
                //get answers
                $result[$key1] = Question::model()->getQuestionsForStatistics('title, question, qid', "parent_qid='$flt[0]' AND language = '{$language}'", 'question_order');
                //$counter2=0;
                ...
                foreach ($result[$key1] as $row) {
                    ...
                    if (isset($_POST[$myfield2])) {
                        echo htmlspecialchars((string) $_POST[$myfield2]);
                    }
                    ...
                    . "</div>\n"
                    . "\t<select name='Q{$flt[0]}_S{$qid}[]' multiple='multiple' class='form-select'>\n";
                    ...
                    for ($i = 1; $i <= 5; $i++) {
                        ...
                        //pre-select
                        if (isset($_POST[$myfield2]) && is_array($_POST[$myfield2]) && in_array($i, $_POST[$myfield2])) {
                            echo " selected='selected' ";
                        }
                        if (isset($_POST[$myfield2]) && $_POST[$myfield2] == $i) {
                            echo " selected='selected' ";
                        }
            ...
            case Question::QT_C_ARRAY_YES_UNCERTAIN_NO: // ARRAY OF YES\No\gT("Uncertain") QUESTIONS
                ...
                foreach ($result[$key1] as $row) {
                    ...
                    //pre-select "yes"
                    if (isset($_POST[$myfield2]) && is_array($_POST[$myfield2]) && in_array("Y", $_POST[$myfield2])) {
                        echo " selected='selected' ";
                    }
                    ...
                    if (isset($_POST[$myfield2]) && is_array($_POST[$myfield2]) && in_array("U", $_POST[$myfield2])) {
                        echo " selected='selected' ";
                    }
                    ...
                    //pre-select "no"
                    if (isset($_POST[$myfield2]) && is_array($_POST[$myfield2]) && in_array("N", $_POST[$myfield2])) {
                        echo " selected='selected' ";
                    }
            ...
            case Question::QT_E_ARRAY_INC_SAME_DEC: // Array of Increase/Same/Decrease questions
                ...
                foreach ($result[$key1] as $row) {
                    ...
                    if (isset($_POST[$myfield]) && is_array($_POST[$myfield2]) && in_array("S", $_POST[$myfield2])) {
                        echo " selected='selected' ";
                    }
                    ...
                    if (isset($_POST[$myfield]) && is_array($_POST[$myfield2]) && in_array("D", $_POST[$myfield2])) {
                        echo " selected='selected' ";
                    }
            ...
            case Question::QT_SEMICOLON_ARRAY_TEXT:  // Array (Text)
                ...
                foreach ($result[$key1] as $key => $row) {
                    ...
                    foreach ($fresult as $frow) {
                        $myfield2 = "T" . $myfield . "_S" . $row['qid'] . "_S" . $frow['qid'];
                        echo "<!-- $myfield2 - ";
                        if (isset($_POST[$myfield2])) {
                            echo htmlspecialchars((string) $_POST[$myfield2]);
                        }
                        ...
                        echo " />&nbsp;"
                        . $flt[3] . " - " . $oStatisticsHelper::_showSpeaker($niceqtext . " " . str_replace("'", "`", $row['question'] . " [" . $frow['question'] . "]") . " - " . $row['title'] . "/" . $frow['title'])
                        . "<br />\n";
            ...
            case Question::QT_COLON_ARRAY_NUMBERS:  // Array (Numbers)
                ...
                if (empty($qidattributes['input_boxes'])) {
                    ...
                    foreach ($result[$key1] as $row) {
                        ...
                        foreach ($fresult as $frow) {
                            $myfield2 = $myfield . "_S" . $row['qid'] . "_S" . $frow['qid'];
                            ...
                            echo " />&nbsp;"
                            . $flt[3] . " - " . $oStatisticsHelper::_showSpeaker($niceqtext . " " . str_replace("'", "`", $row['question'] . " [" . $frow->questionl10ns[$language]->question . "]") . " - " . $row['title'] . "/" . $frow['title'])
                            . "\n"
                            . "</div>\n";
                    ...
                    foreach ($result[$key1] as $row) {
                        ...
                        foreach ($fresult as $frow) {
                            ...
                            */
                            ...
                            //show speaker
                            echo $flt[3] . " - " . $oStatisticsHelper::_showSpeaker($niceqtext . " " . str_replace("'", "`", $row['question'] . " [" . $frow->questionl10ns[$language]->question . "]") . " - " . $row['title'] . "/" . $frow['title'])
                                . "</div>
                                <div class='mb-3 row'>
                                <label for='" . $myfield2 . "' class='col-md-4 form-label'>" . gT("Number greater than:") . "</label>
                                <div class='col-md-6'>"
            ...
            */
            ...
            case Question::QT_H_ARRAY_COLUMN: // Array (By Column)
                ...
                foreach ($result[$key1] as $key => $row) {
                    ...
                    */
                    ...
                    //creating form
                    echo "\t<select name='Q{$flt[0]}_S{$row['qid']}[]' multiple='multiple' class='form-select'>\n";
            ...
            case Question::QT_R_RANKING: // Ranking
                ...
                for ($i = 1; $i <= $columnsCount; $i++) {
                    ...
                    foreach ($subquestions as $subquestion) {
                        ...
                        //pre-select
                        if (isset($_POST[$myfield3]) && is_array($_POST[$myfield3]) && in_array("$subquestion[0]", $_POST[$myfield3])) {
                            echo " selected='selected' ";
                        }
            ...
            case Question::QT_1_ARRAY_DUAL: // Dual scale
                ...
                foreach ($result[$key1] as $row) {
                    ...
                    echo " />&nbsp;"
                    . $flt[3] . " - " . $oStatisticsHelper::_showSpeaker($niceqtext . " [" . str_replace("'", "`", (string) $row[15]) . "] - " . gT("Label") . ": " . $labeltitle)
                    . "</div>\n";
                    ...
                    */
                    ...
                    echo "\t<select name='Q{$flt[0]}_S{$qid}#0[]' multiple='multiple' class='form-select'>\n";
                    ...
                    echo " />&nbsp;"
                    . $flt[3] . " - " . $oStatisticsHelper::_showSpeaker($niceqtext . " [" . str_replace("'", "`", (string) $row[15]) . "] - " . gT("Label") . ": " . $labeltitle2)
                    . "</div>\n";
                    $fresult = Answer::model()->getQuestionsForStatistics('*', "qid='$flt[0]' AND language = '$language' AND scale_id = 1", 'sortorder, code');

                    //this is for debugging only
                    echo "\t<select name='Q{$flt[0]}_S{$qid}#1[]' multiple='multiple' class='form-select'>\n";
            ...
            case Question::QT_M_MULTIPLE_CHOICE:  //M - Multiple choice
                ...
                foreach ($result[$key1] as $row) {
                    ...
                    //pre-check
                    if (isset($_POST[$myfield]) && is_array($_POST[$myfield]) && in_array($row['title'], $_POST[$myfield])) {
                        echo " selected='selected' ";
                    }
            ...
            default:
                ...
                foreach ($result[$key1] as $row) {
                    ...
                    //pre-check
                    if (isset($_POST[$myfield]) && is_array($_POST[$myfield]) && in_array($row->code, $_POST[$myfield])) {
                        echo " selected='selected' ";
                    }
