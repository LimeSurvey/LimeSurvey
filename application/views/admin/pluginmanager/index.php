    ...
    $gridColumns = array(
        ...
        array(// display the activation link
            ...
            'value' => function($data) {
                if ($data['active'] == 0)
                {
                    $output = "<a href='/index.php/admin/pluginmanager/sa/activate?id=" . $data["id"] . "' class='btn btn-default btntooltip'><span class='fa fa-power-off'>&nbsp;</span>".gT('Activate')."</a>";
                } else {
                    $output = "<a href='/index.php/admin/pluginmanager/sa/deactivate?id=" . $data["id"] . "'class='btn btn-warning'><span class='fa fa-power-off'>&nbsp;</span>".gT('Deactivate')."</a>";
                }
                if(count($data['settings'])>0)
                {
                    $output .= "&nbsp;<a href='/index.php/admin/pluginmanager/sa/configure?id=" . $data["id"] . "' class='btn btn-default'><span class='icon-edit'>&nbsp;</span>" . gT('Configure') . "</a>";
                }
                return $output;
            }
        ),
