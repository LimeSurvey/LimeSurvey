...
<?php
    ...
    <?php if(Permission::model()->hasSurveyPermission($surveyid,'surveylocale','update')) { ?>
        <p><button onclick="if (UpdateLanguageIDs(mylangs,'<?php eT("All questions, answers, etc for removed languages will be lost. Are you sure?", "js");?>')) { document.getElementById('surveysettingsaction').value = 'updatesurveysettingsandeditlocalesettings'; $('#addnewsurvey').submit();}" class='standardbtn' ><?php eT("Save & edit survey text elements");?> >></button></p>
    <?php } ?>
    <?php
    }
?>
