...
class QuestionExplorer {
  ...
  renderExplorer() {
    ...
    html += '</button>';
    html += '<button class="btn btn-sm btn-outline-secondary me-2 expand-all-btn" title="' + _UIHelpers_js__WEBPACK_IMPORTED_MODULE_2__["default"].translate('expandAll') + '">';
  ...
  expandAll() {
    var questiongroups = _StateManager_js__WEBPACK_IMPORTED_MODULE_0__["default"].get('questiongroups') || [];
    this.active = questiongroups.map(function (questiongroup) {
      return questiongroup.gid;
    });
    _StateManager_js__WEBPACK_IMPORTED_MODULE_0__["default"].commit('questionGroupOpenArray', this.active);
    this.renderExplorer();
  }
...
class SideMenu {
  ...
  renderSubmenu(menu) {
    ...
    sortedEntries.forEach(menuItem => {
      ...
      html += '<a href="' + _UIHelpers_js__WEBPACK_IMPORTED_MODULE_1__["default"].escapeHtml(href) + '"' + (target ? ' target="' + target + '"' : '') + ' id="sidemenu_' + _UIHelpers_js__WEBPACK_IMPORTED_MODULE_1__["default"].escapeHtml(menuItem.name) + '"' + ' class="list-group-item w-100 ' + linkClass + '"' + ' data-menu-item-id="' + _UIHelpers_js__WEBPACK_IMPORTED_MODULE_1__["default"].escapeHtml(menuItem.id) + '"' + ' data-menu-id="' + _UIHelpers_js__WEBPACK_IMPORTED_MODULE_1__["default"].escapeHtml(menuItem.menu_id) + '">';
      html += '<div class="d-flex ' + _UIHelpers_js__WEBPACK_IMPORTED_MODULE_1__["default"].escapeHtml(menuItem.menu_class || '') + '"' + ' title="' + tooltip + '"' + ' data-bs-toggle="tooltip">';
...
class Sidebar {
  ...
  renderStateToggle(isCollapsed, currentTab, isRTL) {
    ...
    if (!isCollapsed) {
      ...
      html += '<div class="ls-flex-item grow-10 col-12">' + '<ul class="nav nav-tabs" id="adminsidepanel-sidebar-tablist" role="tablist">' + '<li class="nav-item">' + '<a id="adminsidepanel__sidebar--selectorSettingsButton" class="nav-link sidebar-tab-link' + (settingsSelected ? ' active' : '') + '" href="#settings" data-tab="settings" role="tab" aria-controls="sidemenu-container" aria-selected="' + (settingsSelected ? 'true' : 'false') + '" tabindex="' + (settingsSelected ? '0' : '-1') + '">' + _UIHelpers_js__WEBPACK_IMPORTED_MODULE_2__["default"].translate('settings') + '</a>' + '</li>' + '<li class="nav-item">' + '<a id="adminsidepanel__sidebar--selectorStructureButton" class="nav-link sidebar-tab-link' + (structureSelected ? ' active' : '') + '" href="#structure" data-tab="questiontree" role="tab" aria-controls="questionexplorer-container" aria-selected="' + (structureSelected ? 'true' : 'false') + '" tabindex="' + (structureSelected ? '0' : '-1') + '">' + _UIHelpers_js__WEBPACK_IMPORTED_MODULE_2__["default"].translate('structure') + '</a>' + '</li>' + '</ul>' + '</div>';
    } else {
