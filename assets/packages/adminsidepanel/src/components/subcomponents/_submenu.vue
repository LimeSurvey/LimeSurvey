...
export default {
    ...
    methods: {
        ...
        checkSelected(menuItem) { return this.$store.state.lastMenuItemOpen == menuItem.id; },
...
<template>
    <ul class="list-group subpanel col-12" :class="'level-' + (menu.level)">
        <li v-for="(menuItem) in sortedMenuEntries" v-bind:key="menuItem.id">
            <a v-on:click.stop="setActiveMenuItemIndex(menuItem)" :href="menuItem.disabled ? '#' : menuItem.link"
                :target="menuItem.link_external == true ? '_blank' : ''" :id="'sidemenu_' + menuItem.name"
                :aria-current="checkSelected(menuItem) ? 'true' : 'false'"
                :aria-disabled="menuItem.disabled ? 'true' : 'false'" class="list-group-item w-100"
