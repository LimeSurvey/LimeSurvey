(function (factory) {
    ...
    class Tour {
      constructor(options) {
        ...
        if (typeof this._options.sanitizeFunction == "function") {
          ...
          const defaultWhiteList = this._options.framework == "bootstrap4" && $.fn.popover.Constructor.Default.whiteList !== undefined ? $.fn.popover.Constructor.Default.whiteList : this._options.framework == "bootstrap3" && $.fn.popover.Constructor.DEFAULTS.whiteList !== undefined ? $.fn.popover.Constructor.DEFAULTS.whiteList : [];
      ...
      goTo(i) {
        ...
        return this._callOnPromiseDone(promise, e => {
          $(document$1).off("click.tour-" + this._options.name);
          $(document$1).off("keyup.tour-" + this._options.name);
      ...
      showStep(i) {
        ...
        if ({}.toString.call(showDelay) === '[object Number]' && showDelay > 0) {
          ...
          if (step.delayOnElement) {
            ...
            const delayElementLog = getElementForLogging();
      ...
      _redirect(step, i, path) {
        if (LS.ld.isFunction(step.redirect)) {
          ...
          const href = {}.toString.call(step.host) === '[object String]' ? "" + step.host + path : path;
      ...
      _showPopover(step, i) {
        ...
        if ($(document$1).find(".popover.tour-" + this._options.name + ".tour-" + this._options.name + "-" + this.getCurrentStepIndex()).length == 0) {
          ...
          if (step.reflex && !isOrphan) {
            $(step.reflexElement).addClass('tour-step-element-reflex').off(this._reflexEvent(step.reflex) + ".tour-" + this._options.name).on(this._reflexEvent(step.reflex) + ".tour-" + this._options.name, e => {
              if (this._isLast()) {
                return this.next();
              } else {
                return this.end();
      ...
      _template(step, i) {
        const template = this._isOrphan(step) && !LS.ld.isBoolean(step.orphan) ? template = step.orphan : step.template;
      ...
      _reposition($tip, step) {
        ...
        const offsetBottom = $(document$1).height() - tipOffset.top - $tip.outerHeight();
        if (offsetBottom < 0) {
      ...
      _initKeyboardNavigation() {
        ...
        return $(document$1).on("keyup.tour-" + this._options.name, e => {
          if (!e.which) {
            return;
          }
          switch (e.which) {
      ...
      _showOverlayElements(step) {
        ...
        const isRedraw = $(document$1).find(".popover.tour-" + this._options.name + ".tour-" + this._options.name + "-" + this.getCurrentStepIndex()).length == 0;
        ...
        const docHeight = $(document$1).height();
        const docWidth = $(document$1).width();
    ...
    const globalTourObject = function () {
      const getBasedUrls = /(\/index.php)?\?r=admin/.test(window.location.href),
        combineParams = function (params) {
          ...
          const returner = ('/') + LS.ld.reduce(params, (urlParams, value, key) => {
    ...
    const TourLibrary = function () {
      ...
      const _getIsTourActive = () => {
        ...
        initTour = tourName => {
          return new Promise((resolve, reject) => {
            ...
            globalTourObject$1.get(tourName).then(tourObject => {
              _setTourActive(tourName);
              tourObject.onEnd = () => {
        ...
        triggerTourStart = tutorialName => {
          clearActiveTour();
          initTour(tutorialName).then(startedTutorial => {
      ...
      if (_activeTour !== false && typeof _actionActiveTour !== 'function') {
        initTour(_activeTour).then(startedTutorial => {
