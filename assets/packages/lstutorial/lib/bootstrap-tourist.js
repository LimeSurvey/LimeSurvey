...
class Tour {
    constructor(options) {
        ...
        if (typeof (this._options.sanitizeFunction) == "function") {
            ...
            const defaultWhiteList = ((this._options.framework == "bootstrap4" && $.fn.popover.Constructor.Default.whiteList !== undefined) ?
                $.fn.popover.Constructor.Default.whiteList :
                ((this._options.framework == "bootstrap3" && $.fn.popover.Constructor.DEFAULTS.whiteList !== undefined) ?
                    $.fn.popover.Constructor.DEFAULTS.whiteList :
                    []
                )
            );

            const whiteListAdditions = {
            ...
            const whiteList = $.extend(true, {}, defaultWhiteList);
            ...
            $.each(whiteListAdditions, (index, value) => {
            ...
            $.each(this._options.sanitizeWhitelist, (index, value) => {
                whiteList[index] = whiteList[index] || [];
                $.merge(whiteList[index], value);
            });
    ...
    start() {
        ...
        $(window).on("resize.tour-" + this._options.name, () => {
            this.reshowCurrentStep();
        });
        ...
        const promise = this._makePromise(this._options.onStart != null ? this._options.onStart(this) : Promise.resolve());
    ...
    goTo(i) {
        ...
        const promise = this.hideStep();
        return this._callOnPromiseDone(promise, this.showStep, i);
    };

    end() {
        const promise = this.hideStep();
        return this._callOnPromiseDone(promise, (e) => {
            ...
            $(window).off("scroll.tour-" + this._options.name);
    ...
    // hides current step
    hideStep() {
        const step = this.getStep(this.getCurrentStepIndex());
        ...
        this._clearTimer();
        const promise = this._makePromise(step.onHide != null ? step.onHide(this, this.getCurrentStepIndex()) : null);

        const hideStepHelper = (e) => {
            const $element = (!($(step.element).data('bs.popover') || $(step.element).data('popover'))) ? $('body') : $(step.element);
            ...
            $element.removeClass("tour-" + this._options.name + "-element tour-" + this._options.name + "-" + this.getCurrentStepIndex() + "-element").removeData('bs.popover');

            if (step.reflex) {
                $(step.reflexElement).removeClass('tour-step-element-reflex').off((this._reflexEvent(step.reflex)) + ".tour-" + this._options.name);
            }

            this._hideOverlayElement(step);
            this._unfixBootstrapSelectPickerZindex(step);

            // If this step was pointed at a modal, revert changes to the step.element. See the notes in showStep for explanation
            const tmpModalOriginalElement = this._getStepFlag(this.getCurrentStepIndex(), "elementModalOriginal");
            if (tmpModalOriginalElement != null) {
                this._setStepFlag(this.getCurrentStepIndex(), "elementModalOriginal", null);
                step.element = tmpModalOriginalElement;
            }

            if (step.onHidden != null) {
                return step.onHidden(this);
            }
        };

        const hideDelay = step.delay.hide || step.delay;
        if (step.delay != false && hideDelay > 0) {
            this._debug("Wait " + hideDelay + " milliseconds to hide the step " + (this._current + 1));
            window.setTimeout(() => {
                return this._callOnPromiseDone(promise, hideStepHelper);
            }, hideDelay);
        } else {
    ...
    showStep(i) {
        ...
        const skipToPrevious = i < this._current;
        const promise = this._makePromise(step.onShow != null ? step.onShow(this, i) : null);
    ...
    _showOverlayElements(step) {
        ...
        if ($(step.element).length === 0 || this._isOrphan(step)) {
            ...
            const $backdropTop = $('<div class="tour-backdrop top"></div>');
            $backdropTop.offset({
                top: 0,
                left: 0
            });
            ...
            const $backdropLeft = $('<div class="tour-backdrop left"></div>');
            ...
            const $backdropRight = $('<div class="tour-backdrop right"></div>');
            ...
            const $backdropBottom = $('<div class="tour-backdrop bottom"></div>');
