(function (factory) {
    ...
    function openUploadModalDialog() {
      ...
      $('.upload').on('click.lsuploadquestion', function (e) {
        ...
        $('#file-upload-modal-' + fieldname).on('hide.bs.modal.lsuploadquestion', function () {
          var pass;
          window.currentUplaodHandler.saveAndExit(fieldname, show_title, show_comment, pos); // if(document.getElementById(uploaderId).contentDocument) {
    ...
    window.displayUploadedFiles = function (filecount, fieldname, show_title, show_comment) {
      ...
      if (jsonstring !== '') {
        ...
        if (show_title != 0) display += '<th>' + uploadLang.titleFld + '</th>';
        if (show_comment != 0) display += '<th>' + uploadLang.commentFld + '</th>';
