(function (factory) {
  ...
  var UploadQuestionController = /*#__PURE__*/function () {
    function UploadQuestionController(fieldname) {
      _classCallCheck(this, UploadQuestionController);
    ...
    _createClass(UploadQuestionController, [{
      key: "prepareOpenUploadModalDialog",
      value: function prepareOpenUploadModalDialog() {
        var _this = this;
        ...
        $(document).on('shown.bs.modal.lsuploadquestion', '#file-upload-modal-' + this.fieldname, function () {
          var uploadFrame = $('#uploader' + _this.fieldname);
          uploadFrame.load(uploadFrame.data('src'));

          _this.updateMaxHeightModalbody(_this.$el);
        });
        this.$modalEl.off('hide.bs.modal.lsuploadquestion');
        this.$modalEl.on('hide.bs.modal.lsuploadquestion', function () {
          var uploadFrame = $('#uploader' + _this.fieldname);
        ...
        this.$el.on('click.lsuploadquestion', function (e) {
          console.ls.log("File upload modal opening");

          _this.$modalEl.modal('show');
        });
      }
      ...
      key: "updateUploadFrame",
      value: function updateUploadFrame(frameName, heigth) {
      ...
      key: "updateMaxHeightModalbody",
      value: function updateMaxHeightModalbody(modal) {
      ...
      value: function getQueryVariable(variable, url) {
        var vars1 = url.split("/");
        ...
        var vars2 = url.replace(/\&amp;/g, '&').split("&");

        for (var _i = 0; _i < vars.length; _i++) {
          var pair = vars2[_i].split("=");
      ...
      key: "isValueInArray",
      value: function isValueInArray(arr, val) {
        inArray = false;
      ...
      key: "displayUploadedFiles",
      value: function displayUploadedFiles(filecount, fieldname, show_title, show_comment) {
        var _this2 = this;
        ...
        if (jsonstring !== '') {
          ...
          var templateHtml = $('#filerowtemplate_' + this.fieldname).html();
          jsonobj.forEach(function (item, iterator) {
            var imageOrPlaceholder, imageOrPlaceholderHtml, title, comment, name, filepointer;

            if (_this2.isValueInArray(image_extensions, item.ext)) {
              imageOrPlaceholder = "image";
              imageOrPlaceholderHtml = "<img src=\"".concat(uploadurl, "/filegetcontents/").concat(decodeURIComponent(item.filename), "\" class=\"uploaded\" />");
            } else {
            ...
            var rowHtml = _this2.replaceWithObject(templateHtml, {
              imageOrPlaceholder: imageOrPlaceholder,
              imageOrPlaceholderHtml: imageOrPlaceholderHtml,
              title: title,
              comment: comment,
              name: name,
            ...
            $('#' + _this2.fieldname + '_uploadedfiles').find('table>tbody').append(rowHtml);
          });
          $('.trigger_edit_upload_' + this.fieldname).off('click.lsuploadquestion');
          $('.trigger_edit_upload_' + this.fieldname).on('click.lsuploadquestion', function () {
            _this2.$modalEl.modal('show');
          });
        }
      }
    }, {
      key: "replaceWithObject",
      value: function replaceWithObject(templateString, objectWithReplacements) {
        var outString = templateString;

        for (var key in objectWithReplacements) {
          outString = outString.replace(new RegExp("{".concat(key, "}")), objectWithReplacements[key]);
        }
      ...
      key: "showBasic",
      value: function showBasic() {
        $('#basic').show();
      }
    }, {
      key: "hideBasic",
      value: function hideBasic() {
        $('#basic').hide();
      }
    }]);

    return UploadQuestionController;
  }();
  ...
  var uploadHandler = function uploadHandler(qid, options) {
    ...
    var doFileUpload = function doFileUpload() {
      ...
      new AjaxUpload(button, {
        ...
        onSubmit: function onSubmit(file, ext) {
          ...
          var _iterator = _createForOfIteratorHelper(allowed_filetypes),
              _step;
    ...
    var copyJSON = function copyJSON(filecount, fieldname, show_title, show_comment, pos) {
      $('#' + fieldname + '_filecount').val(filecount);
