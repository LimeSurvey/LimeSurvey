...
$(document).on('ready  pjax:scriptcomplete', function(){
    ...
    $('a[data-confirm]').click(function(ev) {
        ...
        if (!$('#dataConfirmModal').length) {
            $('body').append('<div  id="dataConfirmModal" class="modal  fade" role="dialog" aria-labelledby="dataConfirmLabel">  <div class="modal-dialog">    <div class="modal-content">      <div class="modal-header">        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>        <h4 class="modal-title">'+strConfirm+'</h4>      </div>      <div class="modal-body">      </div>      <div class="modal-footer"><button type="button" role="button" class="btn btn-primary" id="dataConfirmOK">'+strOK+'</button><button  type="button" class="btn btn-outline-secondary" data-dismiss="modal" >'+strCancel+'</button>      </div>    </div><!-- /.modal-content -->  </div><!-- /.modal-dialog --></div><!-- /.modal -->');
        }
