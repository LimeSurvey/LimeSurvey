    ...
    function showadminmenu()
        ...
        if(isset($_SESSION['loginID'])) //ADDED to prevent errors by reading db while not logged in.
	        ...
	        $adminmenu .= "\t\t\t\t</div>\n"
                        . "\t\t\t</div>\n"
                        . "\t\t</div>\n";
            $adminmenu .= "<font style='size:1px;line-height:1px;'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</font>"; //CSS Firefox 2 transition fix
...
function getuserlist($outputformat='fullinfoarray')
{
		...
		$usercontrolSameGroupPolicy === true)
	{
		if (isset($myuid))
		{
			...
			$uquery = "SELECT u.* FROM ".db_table_name('users')." AS u, ".db_table_name('user_in_groups')." AS ga ,".db_table_name('user_in_groups')." AS gb WHERE u.uid=$myuid OR (ga.ugid=gb.ugid AND ( (gb.uid=$myuid AND u.uid=ga.uid) OR (u.parent_id=$myuid) ) )";
		}
		else
		{
