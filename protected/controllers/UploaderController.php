...
{
    function actionIndex($sid, $fieldname, $qid, $show_title, $show_comment, $minfiles, $maxfiles)
    {
        $sLanguage = isset(Yii::app()->session['survey_' . $sid]['s_lang']) ? Yii::app()->session['survey_' . $sid]['s_lang'] : "";
        ...
        if ($sFileGetContent != $sFileGetContentFiltered || $sFileName != $sFileNameFiltered || $sFieldName != $sFieldNameFiltered) {// If one seems to be a hack: Bad request
        ...
        if ($sFileGetContent) {
            if (substr($sFileGetContent, 0, 6) == 'futmp_') {
                ...
                $sFileDir = "{$uploaddir}/surveys/{$sid}/files/";
            } else {
                throw new CHttpException(400);// See for debug > 1
            }
        ...
        if ($sMode == "upload") {
            ...
            } else {    // if everything went fine and the file was uploaded successfuly,
    ...
    ';
        $sLangScriptVar = "
            uploadLang = {
                 ...
                 errorMoreAllowed: '" . gT('If you wish, you may upload %s more file(s); else you may return back to survey.',
                'js') . "',
                 errorMaxReached: '" . gT('The maximum number of files has been uploaded. You may return back to survey.',
                'js') . "',
                 errorTooMuch: '" . gT('The maximum number of files has been uploaded. You may return back to survey.',
                'js') . "',
    ...
    ";
        $aSurveyInfo = getSurveyInfo($sid, $sLanguage);
        $oEvent = new PluginEvent('beforeSurveyPage');
        $oEvent->set('surveyId', $sid);
        ...
        $body = '</head><body class="uploader">
            ...
            <p class="uploadmsg">' . sprintf(gT("You can upload %s under %s KB each."),
                $qidattributes['allowed_filetypes'], $qidattributes['max_filesize']) . '</p>
