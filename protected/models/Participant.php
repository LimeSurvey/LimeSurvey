...
class Participant extends ActiveRecord
{
    ...
    public function accessibleTo($userId)
    {
        ...
        $sharedIds = array_map(function (ParticipantShare $model) {
            return $model->participant_id;
        }, ParticipantShare::model()->findAllByAttributes(['share_uid' => $userId]));
        if (!empty($sharedIds)) {
    ...
    {
        $count = Yii::app()->db->createCommand()->select('count(*)')->from('{{participants}}')->join('{{participant_shares}}',
            '{{participant_shares}}.participant_id = {{participants}}.participant_id')->where('owner_uid = :userid')->bindParam(":userid",
            $userid, PDO::PARAM_INT)->queryScalar();

        return $count;
    }


    private function getParticipantsSelectCommand(
        $count = false,
        $attid,
        $search = null,
        $userid = null,
        ...
        foreach ($attid as $iAttributeID => $aAttributeDetails) {
            ...
            array_push($joinValue,
                "LEFT JOIN {{participant_attribute}} attribute" . $iAttributeID . " ON attribute" . $iAttributeID . ".participant_id=p.participant_id AND attribute" . $iAttributeID . ".attribute_id=" . $iAttributeID);
        }
    ...
    function deleteParticipants($rows, $bFilter = true)
    {
        ...
        foreach ($aParticipantsIDChunks as $aParticipantsIDs) {
            ...
            Yii::app()->db->createCommand()->delete(Participant::model()->tableName(),
                array('in', 'participant_id', $aParticipantsIDs));

            // Delete survey links
            Yii::app()->db->createCommand()->delete(SurveyLink::model()->tableName(),
                array('in', 'participant_id', $aParticipantsIDs));
            // Delete participant attributes
            Yii::app()->db->createCommand()->delete(ParticipantAttribute::model()->tableName(),
                array('in', 'participant_id', $aParticipantsIDs));
        }
    }
    ...
    function filterParticipantIDs($aParticipantIDs)
    {
        if (!App()->user->checkAccess('superadmin')) // If not super admin filter the participant IDs first to owner only
        {
            ...
            $aParticipantIDs = Yii::app()->db->createCommand()->select('participant_id')->from(Participant::model()->tableName())->where($aCondition,
                $aParameter)->queryColumn();
        }
    ...
    function deleteParticipantToken($sParticipantsIDs)
    {
        ...
        foreach ($aParticipantsIDChunks as $aParticipantsIDs) {
            $aParticipantsIDs = $this->filterParticipantIDs($aParticipantsIDs);
            $aSurveyIDs = Yii::app()->db->createCommand()->selectDistinct('survey_id')->from('{{survey_links}}')->where(array(
                'in',
                'participant_id',
                $aParticipantsIDs
            ))->queryColumn();
            foreach ($aSurveyIDs as $iSurveyID) {
                if (App()->user->checkAccess('tokens',
                    ['crud' => 'delete', 'entity' => 'survey', 'entity_id' => $iSurveyID])
                ) {
                    $sTokenTable = '{{tokens_' . intval($iSurveyID) . '}}';
                    if (Yii::app()->db->schema->getTable($sTokenTable)) {
                        Yii::app()->db->createCommand()->delete($sTokenTable,
                            array('in', 'participant_id', $aParticipantsIDs));
                    }
                }
            }
            ...
            foreach ($tokens as $key => $value) {
                ...
                if (Yii::app()->db->schema->getTable($tokentable)) {
                    ...
                    if ($datas = Yii::app()->db->schema->getTable($surveytable)) {
                        if (!empty($token['token']) && isset($datas->columns['token']) && App()->user->checkAccess('responses',
                                ['crud' => 'delete', 'entity' => 'survey', 'entity_id' => $iSurveyID])
                        ) //Make sure we have a token value, and that tokens are used to link to the survey
                        {
                            ...
                            Yii::app()->db->createCommand()
                                ...
                                ->bindParam(":token", $gettoken['token'],
                                    PDO::PARAM_STR); // Deletes matching responses from surveys
                        }
                    }
                    if (App()->user->checkAccess('tokens',
                        ['crud' => 'delete', 'entity' => 'survey', 'entity_id' => $iSurveyID])
                        ...
                        Yii::app()->db->createCommand()
                            ->delete('{{tokens_' . intval($value['survey_id']) . '}}', 'participant_id = :pid',
                                array(':pid' => $value['participant_id'])); // Deletes matching token table entries
                    }
                }
            }
            $this->deleteParticipants($sParticipantsIDs, false);
    ...
    function getParticipantsSearchMultiple($condition, $page, $limit)
    {
        ...
        while ($i < $con && $con > 2) {
            if ($i < 3) //Special set just for the first query/condition
            {
                ...
                if ($condition[$i] != '') //This section deals with subsequent filter conditions that have boolean joiner
                {
                    ...
                    if ($condition[$i + 1] == "survey") {
                        $lang = Yii::app()->session['adminlang'];
                        $command->addCondition('participant_id IN (SELECT distinct {{survey_links}}.participant_id FROM {{survey_links}}, {{surveys_languagesettings}} WHERE {{survey_links}}.survey_id = {{surveys_languagesettings}}.surveyls_survey_id AND {{surveys_languagesettings}}.surveyls_language=:lang AND ({{surveys_languagesettings}}.surveyls_title ' . $operator . ' ' . $condition2name . '1 OR {{survey_links}}.survey_id ' . $operator . ' ' . $condition2name . '2))',
                            $booloperator);
                        $command->params = array_merge($command->params, array(
                            ':lang' => $lang,
                            $condition2name . '1' => $condition[$i + 3],
                            $condition2name . '2' => $condition[$i + 3]
                        ));
                    ...
                    {
                        $command->addCondition('participant_id IN (SELECT distinct {{participant_attribute}}.participant_id FROM {{participant_attribute}} WHERE {{participant_attribute}}.attribute_id = ' . $condition1name . ' AND {{participant_attribute}}.value ' . $operator . ' ' . $condition2name . ')',
                            $booloperator);
                        $command->params = array_merge($command->params,
                            array($condition1name => $condition[$i + 1], $condition2name => $condition[$i + 3]));
                    } else {
                        $command->addCondition($condition[$i + 1] . ' ' . $operator . ' ' . $condition2name,
                            $booloperator);
                        $command->params = array_merge($command->params, array($condition2name => $condition[$i + 3]));
                    }
                    $i = $i + 4;
...
//                $command->addCondition('(attribute_id='. $sFieldname . ' AND value ' . $operator . ' '.$param.' )', $booloperator);
    ...
    {
        $userid = App()->user->id;
        $is_owner = Yii::app()->db->createCommand()->select('count(*)')->where('participant_id = :participant_id AND owner_uid = :userid')->from('{{participants}}')->bindParam(":participant_id",
            $participant_id, PDO::PARAM_STR)->bindParam(":userid", $userid, PDO::PARAM_INT)->queryScalar();
        $is_shared = Yii::app()->db->createCommand()->select('count(*)')->where('participant_id = :participant_id AND share_uid = :userid')->from('{{participant_shares}}')->bindParam(":participant_id",
            $participant_id, PDO::PARAM_STR)->bindParam(":userid", $userid, PDO::PARAM_INT)->queryScalar();
    ...
    {
        return Yii::app()->db->createCommand()->select('{{participants}}.*, {{participant_shares}}.*')->from('{{participants}}')->join('{{participant_shares}}',
            '{{participant_shares}}.participant_id = {{participants}}.participant_id')->where('owner_uid = :userid')->bindParam(":userid",
            $userid, PDO::PARAM_INT)->queryAll();
    }
    ...
    {
        return Yii::app()->db->createCommand()->select('{{participants}}.*,{{participant_shares}}.*')->from('{{participants}}')->join('{{participant_shares}}',
            '{{participant_shares}}.participant_id = {{participants}}.participant_id')->queryAll();
    }
     ...
     * */
    function copyCPBDAttributesToTokens(
        $surveyid,
        $mapped,
        $newcreate,
        $participantid,
        ...
        if ($createautomap == "true") {
            foreach ($mapped as $key => $iIDAttributeCPDB) {
                if (is_numeric($iIDAttributeCPDB)) {
                    ...
                    Yii::app()->db
                        ->createCommand()
                        ->update('{{surveys}}', array("attributedescriptions" => json_encode($aAttributes)),
                            'sid = ' . $surveyid);
                }
            }
        }
        ...
        if (!empty($newcreate)) //Create new fields in the tokens table
        {
            foreach ($newcreate as $key => $value) {
                ...
                $attname = Yii::app()->db
                    ...
                    ->join('{{participant_attribute_names_lang}}',
            ...
            Yii::app()->db
                ->createCommand()
                ->update('{{surveys}}',
                    array("attributedescriptions" => $aTokenAttributes),
                    'sid = ' . intval($surveyid)); // load description in the surveys table
        ...
        foreach ($participantid as $key => $sParticipantUID) {
            $writearray = array();
            $participantdata = Yii::app()->db->createCommand()->select('firstname,lastname,email,language,blacklisted')->where('participant_id = :pid')->from('{{participants}}')->bindParam(":pid",
                $sParticipantUID, PDO::PARAM_INT);
            ...
            $sQuery = Yii::app()->db->createCommand()->select('*')->from('{{tokens_' . $surveyid . '}}')
                ...
                ->queryAll();
            if (count($sQuery) > 0) {
                //ls\models\Participant already exists in token table - don't copy
                $duplicate++;
                ...
                if ($overwriteauto == "true") {
                    ...
                    if (!empty($newcreate)) {
                        ...
                        for ($a = 0; $a < $numberofattributes; $a++) {
                            Participant::model()->updateTokenAttributeValue($surveyid, $sParticipantUID,
                                $attributesadded[$a], $attributeidadded[$a]);
                        }
                    }
                    ...
                    if (!empty($mapped)) {
                        foreach ($mapped as $key => $value) {
                            if ($key[10] == 'c') { //We know it's automapped because the 11th letter is 'c'
                                Participant::model()->updateTokenAttributeValue($surveyid, $sParticipantUID, $value,
                                    $key);
                            }
                        }
                    }
        ...
        $returndata = array(
            'success' => $sucessfull,
            'duplicate' => $duplicate,
            'blacklistskipped' => $iBlacklistSkipped,
            'overwriteauto' => $overwriteauto,
            'overwriteman' => $overwriteman
        );
     ...
     *
     * @param int $surveyId ls\models\Survey ID number
    ...
    function updateAttributeValueToken($surveyId, $participantId, $participantAttributeId, $tokenFieldname)
    {
        ...
        if (!empty($value2[$tokenFieldname])) {
            ...
            if ($test['count'] > 0) {
                $sql = Yii::app()->db
                    ->createCommand()
                    ->update('{{participant_attribute}}', array("value" => $value2[$tokenFieldname]),
                        "participant_id='$participantId' AND attribute_id=$participantAttributeId");
            } else {
                $sql = Yii::app()->db
                    ->createCommand()
                ...
                if (count($query) > 0) {
                    ...
                    if ($overwriteman == "true" && !empty($aMapped)) {
                        foreach ($aMapped as $cpdbatt => $tatt) {
                            Participant::model()->updateAttributeValueToken($surveyid, $query[0]['participant_id'],
                                $cpdbatt, $tatt);
                        }
                    }
