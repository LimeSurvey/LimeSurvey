...
class User extends ActiveRecord implements \ls\pluginmanager\iUser
{
     ...
     * Delete user
     *
     * @param int $iUserID The ls\models\User ID to delete
     * @return mixed
     */
    ...
    {
        return Yii::app()->db->createCommand()->select('uid')->from('{{users}}')->where('parent_id = :parent_id')->bindParam(":parent_id",
            $parentid, PDO::PARAM_INT)->queryRow();
    }
     ...
     *
     * @param int $iUserID The ls\models\User ID
    ...
    /**
     * Returns ls\models\User ID common in Survey_Permissions and User_in_groups
     *
     * @access public
    ...
    public function getCommonUID($surveyid, $postusergroupid)
    {
        ...
        return Yii::app()->db->createCommand($query2)->bindParam(":surveyid", $surveyid,
            PDO::PARAM_INT)->bindParam(":postugid", $postusergroupid, PDO::PARAM_INT)->query(); //Checked
    }
