...
* 
* $Id: qanda.php 8450 2010-03-02 21:41:25Z c_schmitz $
*/
...
function do_ranking($ia)
{
	...
	for ($i=1; $i<=$max_answers; $i++)
	{
		...
		$ranklist .= "' />\n";		
		$ranklist .= "<input type=\"button\" tabindex=\"1\" alt=\"".$clang->gT("Remove this item")."\" title=\"".$clang->gT("Remove this item")."\" ";
		if ($i != $existing)
		{
			$ranklist .= "style=\"display:none;background-image:url($imagefiles/cut.gif);height:16px; width:16px;border:none \"";
		}

		$ranklist .= " id=\"cut_{$ia[0]}$i\" onclick=\"deletethis_{$ia[0]}(document.getElementById('RANK_{$ia[0]}$i').value, document.getElementById('fvalue_{$ia[0]}$i').value, document.getElementById('RANK_{$ia[0]}$i').name, this.id)\" onkeydown=\"if(event.keyCode==32){deletethis_{$ia[0]}(document.getElementById('RANK_{$ia[0]}$i').value, document.getElementById('fvalue_{$ia[0]}$i').value, document.getElementById('RANK_{$ia[0]}$i').name, this.id);document.getElementById(CHOICES_{$ia[0]}).focus()}\" /><br />\n";
		$inputnames[]=$myfname;
		$ranklist .= "</td></tr>\n";
	}
    ...
    $choicelist .= " id=\"CHOICES_{$ia[0]}\" onkeydown=\"if(event.keyCode==32){if (this.options.length>0 && this.selectedIndex<0) {this.options[this.options.length-1].selected=true;}; rankthis_{$ia[0]}(this.options[this.selectedIndex].value, this.options[this.selectedIndex].text)}\" onclick=\"if (this.options.length>0 && this.selectedIndex<0) {this.options[this.options.length-1].selected=true;}; rankthis_{$ia[0]}(this.options[this.selectedIndex].value, this.options[this.selectedIndex].text)\" class=\"select\" tabindex=\"0\">\n";
	...
	. "".$clang->gT("Click on the scissors next to each item on the right to remove the last entry in your ranked list or press 'space' ")
	. "\t</font></td>\n"
	. "</tr>\n"
	. "\t</table>\n";
