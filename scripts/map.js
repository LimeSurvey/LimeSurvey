...
$(document).ready(function()
{
  $(".location").each(function(index,element){
    ...
    var question_id = question.substr(2);
    var latLng = $("#answer_"+question_id).val().split(";");
    if ($("#mapservice_"+question_id).val()==1){
      // Google Maps
      if (gmaps[''+question] == undefined) {
        gmaps[''+question] = GMapsInitialize(question_id,latLng[0],latLng[1]);
      }
    }
    else if ($("#mapservice_"+question_id).val()==2){
      // Open Street Map
      if (osmaps[''+question]==undefined) {
        osmaps[''+question] = OSMapInitialize(question_id,latLng[0],latLng[1]);
      }
    }
  });
});
...
function OSMapInitialize(question,lat,lng){
    map = new OpenLayers.Map("map_canvas_" + question);
...
function GMapsInitialize(question,lat,lng) {
  ...
  var map = new google.maps.Map(document.getElementById("map_canvas_" + question), mapOptions);
  ...
  google.maps.event.addListener(map, 'rightclick', function(event) {
    ...
    geocodeAddress(question, event.latLng);
    $("#location_"+question).val(Math.round(event.latLng.lat()*10000)/10000 + " " + Math.round(event.latLng.lng()*10000)/10000);
  });
  ...
  var input = document.getElementById("location_" + question);
  var autocomplete = new google.maps.places.Autocomplete(input, { types: ['geocode'] });
  autocomplete.bindTo('bounds', map);
  google.maps.event.addListener(autocomplete, 'place_changed', function() {
      marker.setVisible(false);
      $("#location_" + question).removeClass('notfound')
      ...
      if (place.geometry) {
        ...
        geocodeAddress(question, place.geometry.location);
        marker.setPosition(place.geometry.location);
      } else {
        // Inform the user that the place was not found and return.
        $("#location_" + question).addClass('notfound')
      }
...
function geocodeAddress(name, pos) {
  ...
  geocoder.geocode({
    ...
    if (boycott.indexOf("2")!=-1) city = '';
    if (boycott.indexOf("3")!=-1) state = '';
    if (boycott.indexOf("4")!=-1) country = '';
    if (boycott.indexOf("5")!=-1) postal = '';
    $("#answer_"+name).val(""+ Math.round(pos.lat()*10000)/10000 + ';' + Math.round(pos.lng()*10000)/10000
...
function resetMap(qID) {
  ...
  var name = question.substr(2);
  var coordinates = $('#question'+qID+' input.coordinates').attr('value');
  var xy = coordinates.split(";");
  if(gmaps[''+question]) {
    var currentMap = gmaps[''+question];
    var marker = gmaps['marker__'+name];
