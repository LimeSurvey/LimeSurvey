...
//SUBMIT
if ($_POST['move'] == " submit " && !$notanswered)
	{
	if ($surveyprivate == "Y")
		{
		$privacy="";
	...
	if ($surveyactive != "Y")
		...
		if (mysql_query($subquery))
			...
			$completed .= "Your survey responses have been recorded.<br /><br />\n";
			$completed .= "<a href='javascript:window.close()'>Close this window</a></font><br />\n";
			if ($_POST['token'])
				{
...
function surveymover()
	...
	if ($_SESSION['step'] && ($_SESSION['step'] == $_SESSION['totalsteps']) && !$presentinggroupdescription && $surveyformat == "A")
		{$surveymover .= "\t\t\t\t\t<input class='submit' type='submit' value=' submit ' name='move' />\n";}
	//$surveymover .= "\t\t\t\t\t<br />\n";
