...
bMoveLanguageSelect=false // Move the language selector to the top
bCloneNavigator=false // Clone the navigator in the header
bMoveIndex=false // Move index in a fixed bix at rigth of the survey
bHeaderFixed=false; // Fix the header
bAutoMove=true; // Avancer automatiquement (peut être modifié au global mais aussi au cas par cas */
...
fixLabelClass(); 
autoSubmit(); // Submit auto with question by question mode for single list choice
...
function autoSubmit(){
	$(document).ready(function(){
		$(".questionbyquestion .list-dropdown select").change(function(){
			if(bAutoMove && $(this).val()!="" && $(this).val()!="-oth-"){
				$("#movenextbtn,#movesubmitbtn").click();
			}
		});
		$(".questionbyquestion .list-radio input[type=radio]").click(function(){
			if(bAutoMove && $(this).val()!="" && $(this).val()!="-oth-"){
				$("#movenextbtn,#movesubmitbtn").click();
			}
		});
...
function removeEMerror(){
	$("span[style*='background-color']").remove();
}
function textToTel(){
	$("input[type=text][name$=tel],input[type=text][name$=Tel],input[type=text][name$=TEL]").attr('type','tel');
	$("input[type=text][name$=email],input[type=text][name$=Email],input[type=text][name$=EMAIL]").attr('type','email');
}
/* /ScoreWay */
...
function fixLabelClass(){
	...
	$(document).on('click','td.checkbox-item [type=checkbox]',function(){
...
/* needConfirmHandler fix (maybe with opacity ?)*/
function needConfirmHandler(){
	$(document).on('click',"[data-confirmedby]", function(event){
		text=$("label[for='"+$(this).attr('data-confirmedby')+"']").text();
		if (confirm(text)) {
			$("#"+$(this).attr('data-confirmedby')).prop('checked',true);
			return true;
		}
...
function doCombobox(){ try { $(".answers-wrapper select").msDropDown(); } catch(e) { } }
