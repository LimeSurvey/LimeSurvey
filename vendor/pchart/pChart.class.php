...
// * Fixed in 1.27 *         { $X2 = $this->GArea_X2 - 1; $Y2 = $this->GArea_Y2 - ($this->GArea_X2 - $X1); }
         ...
         imageline($this->Picture,(int) round($X1),(int) round($Y1),(int) round($X2),(int) round($Y2+1),$LineColor);
        }
      }
    }
     ...
     for($i=1;$i<=$Divisions+1;$i++)
      {
       ...
       if ( $RightScale )
        {
         imagettftext($this->Picture,$this->FontSize,0,(int) round($this->GArea_X2+10),(int) round($YPos+($this->FontSize/2)),$C_TextColor,$this->FontName,$Value);
         if ( $XMin < $this->GArea_X2+15+$TextWidth || $XMin == NULL ) { $XMin = $this->GArea_X2+15+$TextWidth; }
        }
       else
        {
         imagettftext($this->Picture,$this->FontSize,0,(int) round($this->GArea_X1-10-$TextWidth),(int) round($YPos+($this->FontSize/2)),$C_TextColor,$this->FontName,$Value);
     ...
     if ( isset($DataDescription["Axis"]["Y"]) )
      {
       ...
       if ( $RightScale )
        imagettftext($this->Picture,$this->FontSize,90,(int) round($XMin+$this->FontSize),(int) round($TextTop),$C_TextColor,$this->FontName,$DataDescription["Axis"]["Y"]);
       else
        imagettftext($this->Picture,$this->FontSize,90,(int) round($XMin-$this->FontSize),(int) round($TextTop),$C_TextColor,$this->FontName,$DataDescription["Axis"]["Y"]);
      }
         ...
         if ( $Angle == 0 )
          {
           ...
           imagettftext($this->Picture,$this->FontSize,$Angle,(int) round(floor($XPos)-floor($TextWidth/2)),(int) round($YPos),$C_TextColor,$this->FontName,$Value);
          }
         else
          {
           $YPos = $this->GArea_Y2+10+$TextHeight;
           if ( $Angle <= 90 )
            imagettftext($this->Picture,$this->FontSize,$Angle,(int) round(floor($XPos)-$TextWidth+5),(int) round($YPos),$C_TextColor,$this->FontName,$Value);
           else
            imagettftext($this->Picture,$this->FontSize,$Angle,(int) round(floor($XPos)+$TextWidth+5),(int) round($YPos),$C_TextColor,$this->FontName,$Value);
          }
    ...
    if ( isset($DataDescription["Axis"]["X"]) )
      {
       ...
       imagettftext($this->Picture,$this->FontSize,0,(int) round($TextLeft),(int) round($YMax+$this->FontSize+5),$C_TextColor,$this->FontName,$DataDescription["Axis"]["X"]);
      }
    }
       ...
       foreach ( $Data as $Key => $Values )
        {
         if (isset($Data[$Key][$YSerieName]))
          {
           $Value = round($Data[$Key][$YSerieName]);
           if ( $this->VMax < $Value) { $this->VMax = $Value; }
           if ( $this->VMin > $Value) { $this->VMin = $Value; }
          }
        }
       ...
       foreach ( $Data as $Key => $Values )
        {
         if (isset($Data[$Key][$XSerieName]))
          {
           $Value = round($Data[$Key][$XSerieName]);
     ...
     if ( $Mosaic )
      {
       ...
       imagefilledrectangle($this->Layers[0],0,0,(int) round($LayerWidth),(int) round($LayerHeight),$C_White);
       ...
       for($i=0;$i<=$this->DivisionCount;$i++)
        {
         ...
         if ( $i % 2 == 0 )
          {
           imagefilledrectangle($this->Layers[0],1,(int) round($YPos),(int) round($LayerWidth-1),(int) round($LastY),$C_Rectangle);
          }
        }
     ...
     foreach($DataDescription["Description"] as $Key => $Value)
      {
       ...
       @imagettftext($this->Picture,$this->FontSize,0,(int) round($XPos+22),(int) round($YPos+$YOffset),$C_TextColor,$this->FontName,$Value);
     ...
     foreach($Data as $Key => $Value)
      {
       ...
       @imagettftext($this->Picture,$this->FontSize,0,(int) round($XPos+22),(int) round($YPos+$YOffset),$C_TextColor,$this->FontName,$Value2);
       $YOffset = $YOffset + $TextHeight + 4;
       $ID++;
      }
    }
     ...
     if ( $Shadow )
      {
       ...
       imagettftext($this->Picture,$this->FontSize,0,(int) round($XPos+$this->ShadowXDistance),(int) round($YPos+$this->ShadowYDistance),$C_ShadowColor,$this->FontName,$Value);     
      }

     imagettftext($this->Picture,$this->FontSize,0,(int) round($XPos),(int) round($YPos),$C_TextColor,$this->FontName,$Value);     
    }
   ...
   function drawTextBox($X1,$Y1,$X2,$Y2,$Text,$Angle=0,$R=255,$G=255,$B=255,$Align=ALIGN_LEFT,$Shadow=TRUE,$BgR=-1,$BgG=-1,$BgB=-1,$Alpha=100)
    {
     ...
     if ( $Shadow )
      imagettftext($this->Picture,$this->FontSize,$Angle,(int) round($X+1),(int) round($Y+1),$C_ShadowColor,$this->FontName,$Text);     

     imagettftext($this->Picture,$this->FontSize,$Angle,(int) round($X),(int) round($Y),$C_TextColor,$this->FontName,$Text);     
    }
     ...
     if ( $ShowLabel )
      {
       ...
       if ( $ShowOnRight )
        imagettftext($this->Picture,$this->FontSize,0,(int) round($this->GArea_X2+2),(int) round($Y+($this->FontSize/2)),$C_TextColor,$this->FontName,$Label);
       else
        imagettftext($this->Picture,$this->FontSize,0,(int) round($this->GArea_X1+2),(int) round($Y-($this->FontSize/2)),$C_TextColor,$this->FontName,$Label);
      }
    }
   ...
   function setLabel($Data,$DataDescription,$SerieName,$ValueName,$Caption,$R=210,$G=210,$B=210)
    {
     ...
     imagettftext($this->Picture,$this->FontSize,0,(int) round($XPos + 10),(int) round($YPos + $TextOffset),$C_TextColor,$this->FontName,$Caption);
    }
       ...
       if ( isset($Data[$Key][$YSerieName]) && isset($Data[$Key][$XSerieName]) )
        {
         ...
         $X = $this->GArea_X1 + (($X-$this->VMin) * $this->XDivisionRatio);
         ...
         if ( isset($Data[$Key][$Serie]) && is_numeric($Data[$Key][$Serie]))
          {
           ...
           imagettftext($this->Picture,$this->FontSize,0,(int) round($XOffset),(int) round($YOffset),$C_TextColor,$this->FontName,$Value);
          }
         $XPos = $XPos + $this->DivisionWidth;
        }
     ...
     foreach ( $DataDescription["Values"] as $Key2 => $ColName )
      {
       ...
       imagefilledrectangle($this->Layers[$GraphID],0,0,(int) round($LayerWidth),(int) round($LayerHeight),$C_White);
           ...
           if ( is_numeric($Value) )
            {
             ...
             imagefilledrectangle($this->Layers[$GraphID],(int) round($XPos-$XWidth),(int) round($YPos),(int) round($XPos+$XWidth),(int) round($YZero),$C_Graph);
       ...
       if (isset($Data[$i][$DataDescription["Position"]]))
        {
         ...
         imagettftext($this->Picture,$this->FontSize,0,(int) round($X+$XOffset),(int) round($Y+$YOffset),$C_TextColor,$this->FontName,$Label);
        }
      }
     ...
     for ( $t=1; $t<=$MaxValue; $t++)
      {
       ...
       imagettftext($this->Picture,$this->FontSize,0,(int) round($X),(int) round($Y),$C_TextColor,$this->FontName,$t);
      }
    }
       ...
       if ( !($DrawLabels == PIE_NOLABEL) )
        {
         ...
         imagettftext($this->Picture,$this->FontSize,0,(int) round($TX),(int) round($TY),$C_TextColor,$this->FontName,$Caption);
        }
   ...
   function drawFilledRectangle($X1,$Y1,$X2,$Y2,$R,$G,$B,$DrawBorder=TRUE,$Alpha=100,$NoFallBack=FALSE)
    {
     ...
     if ( $Alpha == 100 )
     ...
     else
      {
       ...
       imagefilledrectangle($this->Layers[0],round(1),round(1),(int) round($LayerWidth-1),(int) round($LayerHeight-1),$C_Rectangle);
     ...
     for($i=0;$i<=90;$i=$i+$Step)
      {
       ...
       imageline($this->Picture,(int) round($Xi1),(int) round($Yi1),(int) round($X1+$Radius),(int) round($Yi1),$C_Rectangle);
       imageline($this->Picture,(int) round($X2-$Radius),(int) round($Yi2),(int) round($Xi2),(int) round($Yi2),$C_Rectangle);
       imageline($this->Picture,(int) round($X2-$Radius),(int) round($Yi3),(int) round($Xi3),(int) round($Yi3),$C_Rectangle);
       imageline($this->Picture,(int) round($Xi4),(int) round($Yi4),(int) round($X1+$Radius),(int) round($Yi4),$C_Rectangle);
     ...
     for($i=90;$i<=270;$i=$i+$Step)
      {
       ...
       if ( ($Y1-1) > $Yc - max($Width,$Height) )
        imageline($this->Picture,(int) round($X1),(int) round($Y1-1),(int) round($X2-1),(int) round($Y2-1),$C_Circle);
      }
    }
   ...
   function addBorder($Size=3,$R=0,$G=0,$B=0)
    {
     ...
     imagefilledrectangle($Resampled,0,0,(int) round($Width),(int) round($Height),$C_Background);
     ...
     elseif ( $Mode == "GD" )
      {
       ...
       foreach($this->Errors as $key => $Value)
        {
         imagettftext($this->Picture,$this->ErrorFontSize,0,(int) round($this->XSize-($MaxWidth+15)),(int) round($YPos),$C_TextColor,$this->ErrorFontName,$Value);
